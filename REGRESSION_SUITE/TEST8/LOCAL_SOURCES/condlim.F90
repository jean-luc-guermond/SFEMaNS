...
CONTAINS
  ...
  MODULE FUNCTION source_in_NS_momentum(TYPE_VEC, rr, mode, i, time, Re, ty, density, tempn, concn) RESULT(vv)
    ...
    ELSE
       ...
       ELSE IF (m==0 .AND. TYPE_VEC==3) THEN
          vv = r**2*z*COS(t) &
       ...
       END IF
    END IF
    IF (TYPE_VEC .GE. 5) THEN
       vv =  vv - inputs%gravity_coefficient*temperature_exact(TYPE_VEC-4,rr,m,t) &
            + inputs%gravity_coefficient*tempn(:,TYPE_VEC-4,i)
    END IF
    RETURN
    !===Dummy variables to avoid warning
  ...
  MODULE FUNCTION source_in_temperature(TYPE_VEC, rr, m, t)RESULT(vv)
    ...
    ELSE
       ...
       IF(m==0 .AND. TYPE_VEC==1) THEN
          vv = -r**2*z*SIN(t) &
               + (ur*2*r*z+uz*r**2)*COS(t) &
               - inputs%temperature_diffusivity(1)*(4*z)*COS(t)
       ELSE IF (m==0 .AND. TYPE_VEC==2) THEN
          vv = 0.d0
       ELSE IF (m==1 .AND. TYPE_VEC==1) THEN
          vv = -r**2*z**2*SIN(t) &
               + (ur*2*r*z**2+uz*2*r**2*z)*COS(t) &
               -inputs%temperature_diffusivity(1)*(4*z**2-z**2+2*r**2)*COS(t)
       ...
       ELSE IF (m==2 .AND. TYPE_VEC==2) THEN
          vv = -2*r**2*z**2*SIN(t) &
               + (ur*4*r*z**2+uz*4*r**2*z)*COS(t) &
               - inputs%temperature_diffusivity(1)*(8*z**2-8*z**2+4*r**2)*COS(t)
       ELSE
