  ...
  MODULE FUNCTION source_in_NS_momentum(TYPE_VEC, rr, mode, i, time, Re, ty, density, tempn, concn) RESULT(vv)
    ...
    IF (TYPE_VEC==1.AND.m==0) THEN
       vv = (2*r-1)**2*cos(z+t) & !du/dt
            ...
            -(r-0.5d0)/2 !ROT(u) x u
    ELSE IF (TYPE_VEC==1.AND.m==1) THEN
       vv = 1.d0 !GRAD p
    ELSE IF (TYPE_VEC==1.AND.m==4) THEN
       vv = (r-0.5d0)/2  !ROT(u) x u
    ELSE IF (TYPE_VEC==2.AND.m==2) THEN
       vv = (4*r**2 - 4*r - 11 + 1/r**2)*(r - 0.5d0)*COS(z+t) & !ROT(u) x u
            -(2-1/r)*(6*r-1)*COS(z+t) !ROT(u) x u
    ELSE IF (TYPE_VEC==3.AND.m==2) THEN
       vv = -(2-1/r)**2*(6*r-1)*COS(z+t) !ROT(u) x u
    ELSE IF (TYPE_VEC==4.AND.m==1) THEN
       vv = -1.d0 !GRAD p
    ELSE IF (TYPE_VEC==4.AND.m==4) THEN
       vv = -(r-0.5d0)**2/r  !ROT(u) x u
    ELSE IF (TYPE_VEC==5.AND.m==0) THEN
    ...
    ELSE IF(TYPE_VEC==6.AND.m==2) THEN
