...
CONTAINS
  ...
  MODULE FUNCTION vv_exact(TYPE_VEC,rr,m,t) RESULT(vv)
    ...
    REAL(KIND=8), DIMENSION(SIZE(rr,2))               :: vv
    REAL(KIND=8)                                      :: r, z
    REAL(KIND=8), PARAMETER                           :: eps=1.d-7, radius_in=1.d0, omega=1.d0
    INTEGER                                           :: k
    vv = 0.d0

    DO k=1, SIZE(rr, 2)!H_mesh%np
       r = rr(1, k)
       z = rr(2, k)
       IF ((m == 0) .AND. (r < radius_in + eps)) THEN
           IF (TYPE_VEC == 3) THEN
              vv(k) = r * omega
           ELSE IF (TYPE_VEC == 5) THEN
              vv(k) = radius_in * omega
           END IF
       END IF
    END DO
...
!    REAL(KIND=8) :: eps=1.d-7, radius_in=1.d0, omega=1.d0
  ...
  MODULE FUNCTION Hexact(H_mesh, TYPE_VEC, rr, m, mu_H_field, t) RESULT(vv)
    ...
    INTEGER                                           :: k, m_bessel = 2
    REAL(KIND=8), PARAMETER                           :: k1 = -0.39, R_out = 2.d0, j2 = 5.135622302
    REAL(KIND=8)                                      :: r, z, val_bessel
    ...
    DO k=1, SIZE(rr,2)
       r = rr(1,k)
       z = rr(2, k)
       val_bessel = BESSI(m_bessel, j2*r/R_out)
       IF (ABS(val_bessel) > 1e4) THEN
          WRITE(*,*) "pb at (r,z) = ", r, z, " with value ", val_bessel 
       END IF
       IF (TYPE_VEC == 3) THEN
          vv(k) = -val_bessel * k1 * SIN(k1*z) * r
          !vv(k) = -val_bessel * k1 * SIN(k1*z)
       ELSE IF (TYPE_VEC == 6) THEN
          !IF (r < 1d-2) THEN
          !   vv(k) = +val_bessel * COS(k1*z) / r
          !ELSE
          !   vv(k) = 0.d0
          !END IF
          vv(k) = +val_bessel * COS(k1*z)
       END IF
    END DO
...
!              mag_field%Hn1(:,k,i) = ampl*(x - 0.5d0)
!            ELSE
!              mag_field%Hn1(:,k,i) = ampl
!            END IF
!          ENDIF
       ENDDO
    ENDDO
