  ...
  MODULE FUNCTION vv_exact(TYPE_VEC,rr,m,t) RESULT(vv)
    ...
    IF (m<2) THEN
       DO k = 1, SIZE(rr, 2)
          ...
          !===== RPI SYMMETRIC PART
          IF (TYPE_VEC == 1) THEN
            vv(k) = vv(k) + (-(PI/heigth_vel)*r*(1.d0-r)*COS(2.d0*PI*z/heigth_vel))
          ELSE IF (TYPE_VEC == 3) THEN
            vv(k) = vv(k) - (4.d0*tor_over_pol)*r*(1-r)*SIN(PI*z/heigth_vel) ! this is centro-antisymmetric
          ELSE IF (TYPE_VEC == 5) THEN
            vv(k) = vv(k) + (2 - 3*r)*SIN(2.*PI*z/heigth_vel)
          END IF
          !===== RPI ANTISYMMETRIC PART
          IF (TYPE_VEC == 1) THEN
            vv(k) = vv(k) + alpha*((PI/(2*heigth_vel))*r*(1.d0-r)*COS(PI*(heigth_vel/2-z)/heigth_vel))
          ELSE IF (TYPE_VEC == 3) THEN
            vv(k) = vv(k) + alpha*(4.d0*tor_over_pol)*r*(1-r) ! this is centro-symmetric
          ELSE IF (TYPE_VEC == 5) THEN
            vv(k) = vv(k) + alpha*(2 - 3*r)*SIN(PI*(heigth_vel/2-z)/heigth_vel)
          END IF
       END DO
    END IF
...
!  END FUNCTION vv_exact_rot_axez
  ...
  MODULE FUNCTION temperature_exact(TYPE_VEC,rr,m,t) RESULT (vv)
    ...
    REAL(KIND=8)                                      :: r, z
    REAL(KIND=8), PARAMETER                           :: eps=1.d-7!, tor_over_pol=0.73d0, alpha=0.d0, PI=ACOS(-1.d0)
    INTEGER                                           :: k
    vv = 0.d0
    ...
    IF (m<2) THEN
       DO k = 1, SIZE(rr, 2)
          ...
          !===== RPI SYMMETRIC PART
          IF (TYPE_VEC == 1) THEN
            vv(k) = vv(k) + (-(PI/heigth_temp)*r*(1.d0-r)*COS(2.d0*PI*z/heigth_temp))
          END IF
          !===== RPI ANTISYMMETRIC PART
          IF (TYPE_VEC == 1) THEN
            vv(k) = vv(k) + alpha*((PI/(2*heigth_temp))*r*(1.d0-r)*COS(PI*(heigth_temp/2-z)/heigth_temp))
          END IF
       END DO
    END IF
  ...
  MODULE FUNCTION concentration_exact(TYPE_VEC,rr,m,t) RESULT (vv)
    ...
    IF (m<2) THEN
       DO k = 1, SIZE(rr, 2)
          ...
          !===== RPI SYMMETRIC PART
          IF (TYPE_VEC == 1) THEN
            vv(k) = vv(k) + (-(PI/heigth_conc)*r*(1.d0-r)*COS(2.d0*PI*z/heigth_conc))
          END IF
          !===== RPI ANTISYMMETRIC PART
          IF (TYPE_VEC == 1) THEN
            vv(k) = vv(k) + alpha*((PI/(2*heigth_conc))*r*(1.d0-r)*COS(PI*(heigth_conc/2-z)/heigth_conc))
          END IF
       END DO
    END IF
  ...
  MODULE FUNCTION Vexact(m, H_mesh) RESULT(vv)  !Set uniquement a l'induction
    ...
    ! DO k=1, H_mesh%np
    !    rr = H_mesh%rr(1, k)
    !    IF ((m == 0) .AND. (rr < radius_in + eps)) THEN
    !       vv(k, 3) = rr * omega
  ...
  MODULE FUNCTION Hexact(H_mesh, TYPE_VEC, rr, m, mu_H_field, t) RESULT(vv)
    ...
    IF (m<2) THEN
       DO k = 1, SIZE(rr, 2)
          ...
          !===== RPI SYMMETRIC PART
          IF (TYPE_VEC == 1) THEN
            vv(k) = vv(k) + (-(PI/heigth_mag)*r*(1.d0-r)*COS(2.d0*PI*z/heigth_mag))
          ELSE IF (TYPE_VEC == 3) THEN
            vv(k) = vv(k) - (4.d0*tor_over_pol)*r*(1-r)*SIN(PI*z/heigth_mag) ! this is centro-antisymmetric
          ELSE IF (TYPE_VEC == 5) THEN
            vv(k) = vv(k) + (2 - 3*r)*SIN(2.*PI*z/heigth_mag)
          END IF
          !===== RPI ANTISYMMETRIC PART
          IF (TYPE_VEC == 1) THEN
            vv(k) = vv(k) + alpha*((PI/(2*heigth_mag))*r*(1.d0-r)*COS(PI*(heigth_mag/2-z)/heigth_mag))
          ELSE IF (TYPE_VEC == 3) THEN
            vv(k) = vv(k) + alpha*(4.d0*tor_over_pol)*r*(1-r) ! this is centro-symmetric
          ELSE IF (TYPE_VEC == 5) THEN
            vv(k) = vv(k) + alpha*(2 - 3*r)*SIN(PI*(heigth_mag/2-z)/heigth_mag)
          END IF
       END DO
    END IF
  ...
  MODULE FUNCTION Phiexact(TYPE_VEC, rr, m, mu_phi,t) RESULT(vv)
    ...
    IF (m<2) THEN
       DO k = 1, SIZE(rr, 2)
          ...
          !===== RPI SYMMETRIC PART
          IF (TYPE_VEC == 1) THEN
            vv(k) = vv(k) + (-(PI/heigth_phi)*r*(1.d0-r)*COS(2.d0*PI*z/heigth_phi))
          END IF
          !===== RPI ANTISYMMETRIC PART
          IF (TYPE_VEC == 1) THEN
            vv(k) = vv(k) + alpha*((PI/(2*heigth_phi))*r*(1.d0-r)*COS(PI*(heigth_phi/2-z)/heigth_phi))
          END IF
       END DO
    END IF
  ...
  MODULE SUBROUTINE init_maxwell(H_mesh, phi_mesh, time, dt, mu_H_field, mu_phi, &
    ...
    mag_field%Hn = mag_field%Hn1
    mag_field%phin = mag_field%phin1
