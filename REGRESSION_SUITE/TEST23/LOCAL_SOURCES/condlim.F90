...
CONTAINS
 ...
 END FUNCTION concentration_exact
  ...
  MODULE FUNCTION Hexact(H_mesh,TYPE_VEC, rr, m, mu_H_field, t) RESULT(vv)
    ...
    REAL(KIND=8), DIMENSION(SIZE(rr,2))               :: vv
    INTEGER                                           :: n, mm
    INTEGER,      DIMENSION(SIZE(rr,2))               :: id
  ...
  MODULE FUNCTION Jexact_gauss(TYPE_VEC, rr, m, mu_phi, sigma, mu_H, t, mesh_id, opt_B_ext) RESULT(vv)
    ...
    IF ((m==0) .AND. (mesh_id==2) .AND. (TYPE_VEC==3)) THEN
       !J_theta
...
!    REAL(KIND=8), DIMENSION(:,:,:), INTENT(OUT):: phin, phin1
    ...
    RETURN
    !===Dummy variables to avoid warning
    nd=H_mesh%np; nd=phi_mesh%np; rd=mu_H_field(1); rd=mu_phi; nd=SIZE(list_mode)
    !===Dummy variables to avoid warning
  ...
  MODULE FUNCTION mu_bar_in_fourier_space(H_mesh,nb,ne,pts,pts_ids) RESULT(vv)
    ...
    ELSE
       vv=mu_bar_in_fourier_space_anal_T23(H_mesh,nb,ne,pts)
    END IF
    RETURN
    !===Dummy variables to avoid warning
