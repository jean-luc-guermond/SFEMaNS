...
! VB 06/02/2026
  USE def_type_field
! VB 06/02/2026
  REAL(KIND=8) :: test11_Ri=7/13d0, test11_Ro=20/13d0, test11_Rossby=1.d-4
...
CONTAINS
  ...
  MODULE FUNCTION source_in_NS_momentum(TYPE_VEC, rr, mode, i, time, Re, ty, density, tempn, concn) RESULT(vv)
    ...
    IF (TYPE_VEC==1) THEN
       vv = inputs%gravity_coefficient*tempn(:,1,i)*r
    ELSE IF (TYPE_VEC==2) THEN
       vv = inputs%gravity_coefficient*tempn(:,2,i)*r
    ELSE IF (TYPE_VEC==5) THEN
       vv = inputs%gravity_coefficient*tempn(:,1,i)*z
    ELSE IF (TYPE_VEC==6) THEN
       vv = inputs%gravity_coefficient*tempn(:,2,i)*z
    ELSE
       vv = 0.d0
    END IF
    RETURN
...
!    REAL(KIND=8), DIMENSION(:,:,:), INTENT(OUT):: phin, phin1
    ...
    DO i = 1, SIZE(list_mode)
       ...
       END DO
       mag_field%Hn(:,1,i) = normalization*(Brho*SIN(theta) + Btheta*COS(theta))
       mag_field%Hn(:,3,i) = normalization*Bphi
       mag_field%Hn(:,5,i) = normalization*(Brho*COS(theta) - Btheta*SIN(theta))
    END DO
    mag_field%Hn1 = mag_field%Hn
    time = 0.d0
    CALL mag_field%set_time(time)
    mag_field%phin = 0.d0
    mag_field%phin1= 0.d0
    RETURN
