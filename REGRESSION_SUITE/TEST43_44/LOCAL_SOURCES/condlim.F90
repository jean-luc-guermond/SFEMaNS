...
CONTAINS
  ...
  MODULE FUNCTION Hexact(H_mesh, TYPE_VEC, rr, m, mu_H_field, t) RESULT(vv)
    ...
    IF ((m==0).AND.(TYPE_VEC==3)) THEN
       DO n= 1, SIZE(rr,2)
          IF ((rr(1,n).LE.user%Rcc) .AND. (rr(2,n).GT.0.d0)) THEN
             vv(n) = user%J0*rr(1,n)*(user%Rmax**2)/(2.d0*user%Rcc**2)
          ELSE IF ((rr(1,n).GT.user%Rcc) .AND. (rr(2,n).GT.0.d0)) THEN
             vv(n) = user%J0*(user%Rmax**2)/(2.d0*rr(1,n))
          ELSE
             vv(n) = user%J0*rr(1,n)/2.d0
          ENDIF
       ENDDO
    END IF
    RETURN
...
!!$       DO i=1, SIZE(list_mode)
!!$          mode = list_mode(i)
!!$          Hn(:,k,i) = Hexact(H_mesh, k, H_mesh%rr, mode, mu_H_field, time)
!!$       ENDDO
!!$    ENDDO
!!$    Hn1=Hn
!!$
...
!!$    time = 0.d0
  ...
  MODULE FUNCTION Derivative_of_potential_from_rhoLi(delta_rhoLi_phys) RESULT(vv)
    ...
    vv = - inputs%pot_slope * inputs%MA/inputs%MB * (inputs%rho_0percent/((delta_rhoLi_phys+65.d0)**2)) /&
         (1  + inputs%MA/inputs%MB * (inputs%rho_0percent/(delta_rhoLi_phys+65.d0) - inputs%xi - 1))**2
    RETURN
  ...
  MODULE FUNCTION molar_fraction_from_concentration(delta_rhoLi_phys) RESULT(vv)
    ...
    vv = 1.d0/(1  + inputs%MA/inputs%MB * (inputs%rho_0percent/(delta_rhoLi_phys+65.d0) - inputs%xi - 1))
    RETURN
