...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  FUNCTION Hexact_init(TYPE_VEC, rr, m, mu_H_field, t) RESULT(vv)
    IMPLICIT NONE
    INTEGER     ,                        INTENT(IN)   :: TYPE_VEC
    REAL(KIND=8), DIMENSION(:,:),        INTENT(IN)   :: rr
    ...
    !===Dummy variables to avoid warning
    nd=TYPE_VEC; nd=SIZE(rr,1); nd=m; rd=t; nd=SIZE(mu_H_field)
    !===Dummy variables to avoid warning
...
!    REAL(KIND=8), DIMENSION(:,:,:), INTENT(OUT):: Hn, Hn1
 !   REAL(KIND=8), DIMENSION(:,:,:), INTENT(OUT):: phin, phin1
    ...
    DO k=1,6
       DO i=1, m_max_c
          mode = list_mode(i)
          mag_field%Hn1(:,k,i) = Hexact_init(k, H_mesh%rr, mode, mu_H_field, time)
          IF (k<3) THEN
             mag_field%phin1(:,k,i) = Phiexact(k, phi_mesh%rr, mode, mu_phi, time)
          ENDIF
       ENDDO
    ENDDO
    time = time + dt
    DO k=1,6
       DO i=1, m_max_c
          mode = list_mode(i)
          mag_field%Hn(:,k,i) = Hexact_init(k, H_mesh%rr, mode, mu_H_field, time)
          IF (k<3) THEN
             mag_field%phin(:,k,i) = Phiexact(k, phi_mesh%rr, mode, mu_phi, time)
          ENDIF
       ENDDO
    ENDDO
