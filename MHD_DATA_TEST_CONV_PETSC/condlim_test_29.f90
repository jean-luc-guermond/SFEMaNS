...
!!$  PUBLIC :: H_B_quasi_static
  ...
  REAL(KIND=8),  PRIVATE,PARAMETER   :: pi = 3.14159265358979323846d0
  REAL(KIND=8),  PRIVATE ,PARAMETER   :: twopi=2*pi
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  REAL(KIND=8),  PARAMETER, PUBLIC:: disk_radius=0.75d0, hole_radius=0.1d0 
  ...
  REAL(KIND=8),  PUBLIC       :: omega_Vol= pi*(1.0)*(2.0) !pi*r^2*h
  ...
  REAL(KIND=8),  PUBLIC, PARAMETER::  disk_r = disk_radius, disk_rp= disk_r- wjump
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
!!$  END SUBROUTINE init_level_set
  ...
  FUNCTION source_in_NS_momentum(TYPE, rr, mode, i, time, Re, ty, opt_density, opt_tempn) RESULT(vv)
    ...
    n=TYPE; n=SIZE(rr,1); n=mode; n=i; r=time; r=Re; np=ty
    !===Dummies variables to avoid warning
...
!!$  END FUNCTION H_B_quasi_static
  ...
  FUNCTION Hexact(H_mesh,TYPE, rr, m, mu_H_field, t) RESULT(vv) 
    ...
    !===Dummies variables to avoid warning
    n=H_mesh%np; n=TYPE; n=SIZE(rr,1); n=m; r=mu_H_field(1); r=t
    !===Dummies variables to avoid warning
  END FUNCTION Hexact
  ...
  FUNCTION Jexact_gauss(TYPE, rr, m, mu_phi, sigma, mu_H, t, mesh_id, opt_B_ext) RESULT(vv) 
    ...
    !===Dummies variables to avoid warning
    n=SIZE(rr,1); r=mu_phi; r=sigma; r=mu_H; r=t; n=TYPE; n=m; n=mesh_id
  ...
  FUNCTION Eexact_gauss(TYPE, rr, m, mu_phi, sigma, mu_H, t) RESULT(vv)
    ...
    !===Dummies variables to avoid warning
    n=SIZE(rr,1); r=mu_phi; r=sigma; r=mu_H; r=t; n=TYPE; n=m
    !===Dummies variables to avoid warning
  ...
  FUNCTION Hexact_init(TYPE, rr, m, mu_H_field, t) RESULT(vv) 
    ...
    !===Dummies variables to avoid warning
    n=SIZE(rr,1); r=mu_H_field(1); r=t
    !===Dummies variables to avoid warning
