...
!!$  END SUBROUTINE init_level_set
  ...
  FUNCTION source_in_NS_momentum(TYPE, rr, mode, i, time, Re, ty, opt_density, opt_tempn) RESULT(vv)
    ...
    IF (PRESENT(opt_density)) CALL error_petsc('density should not be present for test 24') 
    IF (PRESENT(opt_tempn)) CALL error_petsc('temperature should not be present for test 24') 
    ...
    IF (TYPE==1.AND.m==0) THEN 
       vv = (2*r-1)**2*cos(z+t) & !du/dt
            + 2*r*z**3*COS(t) & !GRAD p
                                !-(1/Re)*(sin(z+t)*(12*r**2-1)/r**2-(2-1/r)*(6*r-1)*cos(z+t)) & !viscous
            -(1/Re)*sin(z+t)*(-4*r**4 +4*r**3+11*r**2-1)/r**2 & ! viscous GOOD
            +(2*r-1)/r**3*(6*r-1)*cos(z+t)**2*(4*r**4-4*r**3-11*r**2+1) & !ROT(u) x u
            -(r-0.5d0)/2 !ROT(u) x u
    ELSE IF (TYPE==1.AND.m==1) THEN
       vv = 1.d0 !GRAD p
    ELSE IF (TYPE==1.AND.m==4) THEN
       vv = (r-0.5d0)/2  !ROT(u) x u
    ELSE IF (TYPE==2.AND.m==2) THEN 
       vv = (4*r**2 - 4*r - 11 + 1/r**2)*(r - 0.5d0)*COS(z+t) & !ROT(u) x u
            -(2-1/r)*(6*r-1)*COS(z+t) !ROT(u) x u
    ELSE IF (TYPE==3.AND.m==2) THEN
       vv = -(2-1/r)**2*(6*r-1)*COS(z+t) !ROT(u) x u
    ELSE IF (TYPE==4.AND.m==1) THEN
       vv = -1.d0 !GRAD p
    ELSE IF (TYPE==4.AND.m==4) THEN
       vv = -(r-0.5d0)**2/r  !ROT(u) x u
    ELSE IF (TYPE==5.AND.m==0) THEN
       vv = -(2-1/r)*(6*r-1)*sin(z+t) & !du/dt
            + 3*r**2*z**2*COS(t) & !GRAD p
            +(1/Re)*cos(z+t)*(12*r**4-8*r**3-11*r**2-1)/r**3 & !viscous
            -(2*r-1)**2*sin(z+t)*cos(z+t)*(4*r**4-4*r**3-11*r**2+1)/r**2  !ROT(u) x u
    ELSE IF(TYPE==6.AND.m==2) THEN 
       vv = -(1.d0/Re)*(-3/r + 2/r**2) & !viscous
            +(2*r-1)**2*SIN(z+t) !ROT(u) x u
    ELSE
       vv = 0.d0
    END IF
    RETURN
...
!!$  END FUNCTION level_set_exact
  ...
  FUNCTION penal_in_real_space(mesh,rr_gauss,angles,nb_angles,nb,ne,time) RESULT(vv)
    ...
    !===Dummies variables to avoid warning
    n=mesh%np; n=SIZE(angles); n=nb_angles; r=time
    !===Dummies variables to avoid warning
