...
!!$  END SUBROUTINE init_level_set
  ...
  FUNCTION source_in_NS_momentum(TYPE, rr, mode, i, time, Re, ty, opt_density, opt_tempn) RESULT(vv)
    ...
    IF (PRESENT(opt_density)) CALL error_petsc('density should not be present for test 1') 
    IF (PRESENT(opt_tempn)) CALL error_petsc('temperature should not be present for test 1') 
    ...
    IF (m==0 .OR. m==2) THEN
       ...
       Rot(:,2) =-m1*r*z**3*(4-m1)&
            -6*r**3*z+9*(r*z)**2          
       Rot(:,3) = 3*m1*(r*z)**2-6*r**3*z  &
            -2*r*z**3*(4-m1)
       Rot(:,4) =-3*m1*(r*z)**2-6*r**3*z  &
            -2*r*z**3*(4+m1)
       Rot(:,5) = 12*(r*z)**2-9*r*z**3 + &
            m1*(m1*r*z**3+3*(r*z)**2)
       Rot(:,6) = 12*(r*z)**2-9*r*z**3 + &
            m1*(m1*r*z**3-3*(r*z)**2)          
       Rot = Rot*COS(t)
    ...
    IF (TYPE == 1) THEN
       LAP(:) = m*4*z**3 - 27*r*z**2 + m*6*r**2*z - 6*r**3 &
            -m**2*vt(:,1) - 2*m*vt(:,4) - vt(:,1)
       TEMP(:) = V(:,1)
       GPRESS(:) = 2*r*z**3*COS(t)
    ELSEIF (TYPE == 2) THEN
       LAP(:) =-m*4*z**3 - 27*r*z**2 - m*6*r**2*z - 6*r**3 &
       ...
       GPRESS(:) = 2*r*z**3*COS(t)
    ELSEIF (TYPE == 3) THEN
       LAP(:) = 3*(9*r*z**2 - 4*z**3  + 2*r**3- 6*r**2*z) &
            -m**2*vt(:,3) + 2*m*vt(:,2) -vt(:,3)
       TEMP(:) = V(:,3)
       GPRESS(:) = m*r*z**3*COS(t)
    ELSEIF (TYPE == 4) THEN
       LAP(:) = 3*(9*r*z**2 - 4*z**3  + 2*r**3- 6*r**2*z) &
       ...
       GPRESS(:) = -m*r*z**3*COS(t)
    ELSEIF (TYPE == 5) THEN
       LAP(:) = (4-m)*(4*z**3  + 6*r**2*z) -m**2*vt(:,5)
       TEMP(:) = V(:,5)
       GPRESS(:) = 3*r**2*z**2*COS(t)
    ELSEIF (TYPE == 6) THEN
       LAP(:) = (4+m)*(4*z**3  + 6*r**2*z) -m**2*vt(:,6)
       TEMP(:) = V(:,6)
       GPRESS(:) = 3*r**2*z**2*COS(t)
    ENDIF
...
!!$  END FUNCTION imposed_velocity_by_penalty
  ...
  FUNCTION pp_exact(TYPE,rr,m,t) RESULT (vv)
    ...
    ELSE
       CALL error_petsc('pp_exact: TYPE should be equal to 1 or 2')
    END IF
    RETURN
