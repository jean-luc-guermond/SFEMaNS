...
!!$  END SUBROUTINE init_level_set
  ...
  FUNCTION source_in_NS_momentum(TYPE, rr, mode, i, time, Re, ty, opt_density, opt_tempn) RESULT(vv)
    ...
    REAL(KIND=8), DIMENSION(SIZE(rr,2))                  :: vv, r, z, nu
    REAL(KIND=8)                                         :: nu_bar
    INTEGER          :: m
    REAL(KIND=8)     :: d
    CHARACTER(LEN=2) :: np

    IF (PRESENT(opt_density)) CALL error_petsc('density should not be present for test 36')
    IF (PRESENT(opt_tempn))   m=i
