...
!!$  END SUBROUTINE init_temperature
  ...
  FUNCTION source_in_NS_momentum(TYPE, rr, mode, i, time, Re, ty, opt_density, opt_tempn) RESULT(vv)
    ...
    IF (PRESENT(opt_tempn)) CALL error_petsc('temperature should not be present for test 39')

    vv = 0.d0*Re
    IF (mode==0) THEN
       IF (TYPE==1) THEN
          DO n = 1, SIZE(rr,2)
             IF (rr(2,n).GE.beta*time-0.5d0) THEN
                vv(n)=-inputs%density_fluid(1)*rr(1,n)**3*SIN(alpha*time-rr(2,n))**2 &
                     + 2*rr(1,n)*rr(2,n)**3*COS(time)
             ELSE
                vv(n)=-inputs%density_fluid(2)*rr(1,n)**3*SIN(alpha*time-rr(2,n))**2 &
                     + 2*rr(1,n)*rr(2,n)**3*COS(time)
             END IF
          END DO
       ELSE IF (TYPE==3) THEN
          DO n = 1, SIZE(rr,2)
             IF (rr(2,n).GE.beta*time-0.5d0) THEN
                vv(n)= - 1.d0/inputs%Re*SIN(alpha*time-rr(2,n))*(3.d0-rr(1,n)**2) &
                     + (alpha-beta)*inputs%density_fluid(1)*rr(1,n)**2*COS(alpha*time-rr(2,n))
             ELSE
                vv(n)= - 1.d0/inputs%Re*SIN(alpha*time-rr(2,n))*(3.d0-rr(1,n)**2) &
                     + (alpha-beta)*inputs%density_fluid(2)*rr(1,n)**2*COS(alpha*time-rr(2,n))
             END IF
          END DO
       ELSE IF (TYPE==5) THEN
          DO n = 1, SIZE(rr,2)
             vv(n) = 3*rr(1,n)**2*rr(2,n)**2*COS(time)
          END DO
       ELSE
          vv=0.d0
       END IF
