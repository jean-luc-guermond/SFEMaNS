...
CONTAINS
  ...
  FUNCTION mu_bar_in_fourier_space_anal_T27(H_mesh,nb,ne,pts,pts_ids) RESULT(vv)
    USE def_type_mesh
    USE input_data
    USE my_util
    IMPLICIT NONE
    ...
    RETURN
  END FUNCTION mu_bar_in_fourier_space_anal_T27

  !===Analytical mu_in_fourier_space (if needed)
  FUNCTION grad_mu_bar_in_fourier_space_anal_T27(pt,pt_id) RESULT(vv)
    USE input_data
    USE my_util
    IMPLICIT NONE
    ...
    n=pt_id(1)
    !===Dummies variables to avoid warning
  END FUNCTION grad_mu_bar_in_fourier_space_anal_T27

  FUNCTION mu_in_real_space_anal_T27(H_mesh,angles,nb_angles,nb,ne,time) RESULT(vv)
    USE def_type_mesh
    IMPLICIT NONE
    TYPE(mesh_type)                            :: H_mesh
    ...
    END DO
  END FUNCTION mu_in_real_space_anal_T27

  FUNCTION f_test_T27(r,z) RESULT(vv)
    IMPLICIT NONE
    ...
    RETURN
  END FUNCTION f_test_T27

  FUNCTION f_test_wtime_T27(r,z,t) RESULT(vv)
    IMPLICIT NONE
    REAL(KIND=8), DIMENSION(:), INTENT(IN) :: r, z
    ...
    RETURN
  END FUNCTION f_test_wtime_T27

  FUNCTION dfdr_test_T27(r,z) RESULT(vv)
    IMPLICIT NONE
    REAL(KIND=8), INTENT(IN):: r, z
    REAL(KIND=8)            :: vv
    vv = 3 * b_factor_T27 * (z**2-1)**3 * (r*(1-r))**2 * (1-2*r)
    RETURN
  END FUNCTION dfdr_test_T27

  FUNCTION dfdr_test_wtime_T27(r,z, t) RESULT(vv)
    IMPLICIT NONE
    REAL(KIND=8), INTENT(IN):: r, z,t
    REAL(KIND=8)            :: vv
    vv = 3 * b_factor_T27 * COS(omega_T27*t) * (z**2-1)**3 * (r*(1-r))**2 * (1-2*r)
    RETURN
  END FUNCTION dfdr_test_wtime_T27

  FUNCTION dfdz_test_T27(r,z) RESULT(vv)
    IMPLICIT NONE
    REAL(KIND=8), INTENT(IN):: r, z
    REAL(KIND=8)            :: vv
    vv = 3*b_factor_T27*(r*(1-r))**3*(z**2-1)**2*(2*z)
    RETURN
  END FUNCTION dfdz_test_T27

  FUNCTION dfdz_test_wtime_T27(r,z,t) RESULT(vv)
    IMPLICIT NONE
    REAL(KIND=8), INTENT(IN):: r, z,t
    REAL(KIND=8)            :: vv
    vv = 3*b_factor_T27*COS(omega_T27*t) *(r*(1-r))**3*(z**2-1)**2*(2*z)
    RETURN
  END FUNCTION dfdz_test_wtime_T27
