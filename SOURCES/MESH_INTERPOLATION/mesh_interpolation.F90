...
#include "petsc/finclude/petsc.h"
      ...
      IF (if_conc) THEN
         ...
         IF (check_plt) THEN
            CALL plot_const_p1_label(conc_mesh_glob%jj, conc_mesh_glob%rr, list_inter_conc, 1.d0 * conc_mesh_glob%i_d, 'conc.plt')
         END IF
      END IF
      IF (if_momentum) THEN
         ...
         IF (check_plt) THEN
            CALL plot_const_p1_label(vv_mesh_glob%jj, vv_mesh_glob%rr, list_inter_conc, 1.d0 * vv_mesh_glob%i_d, 'vv.plt')
         END IF
      END IF
      IF (if_energy) THEN
         CALL load_dg_mesh_free_format(directory_m, file_name_m, list_dom_temp, list_inter_conc, 2, temp_mesh_glob, is_form_m)
         IF (check_plt) THEN
            CALL plot_const_p1_label(temp_mesh_glob%jj, temp_mesh_glob%rr, list_inter_conc, 1.d0 * temp_mesh_glob%i_d, 'temp.plt')
         END IF
      END IF
      IF (if_induction) THEN
         ...
         CALL load_dg_mesh_free_format(directory_m, file_name_m, list_dom_phi, list_inter_conc, type_fe_phi, phi_mesh_glob, is_form_m)
         IF (check_plt) THEN
