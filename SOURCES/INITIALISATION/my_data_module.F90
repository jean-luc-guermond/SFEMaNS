...
!!$       END IF
       ...
       IF (test) THEN
          READ(21, *) inputs%my_par_vv_vect%strong_thr
       ELSE
          inputs%my_par_vv_vect%strong_thr = '0.7'
       END IF

       !==========Solver parameters for velocity scalar==========!
       CALL find_string(21, '===Maximum number of iterations for scalar velocity solver', test)
       IF (test) THEN
          READ(21, *) inputs%my_par_vv_scal%it_max
       ELSE
          inputs%my_par_vv_scal%it_max = inputs%my_par_vv_vect%it_max
       END IF
       CALL find_string(21, '===Relative tolerance for scalar velocity solver', test)
       IF (test) THEN
          READ(21, *) inputs%my_par_vv_scal%rel_tol
       ELSE
          inputs%my_par_vv_scal%rel_tol = inputs%my_par_vv_vect%rel_tol
       END IF
       CALL find_string(21, '===Absolute tolerance for scalar velocity solver', test)
       IF (test) THEN
          READ(21, *) inputs%my_par_vv_vect%abs_tol
       ELSE
          inputs%my_par_vv_scal%abs_tol = inputs%my_par_vv_vect%abs_tol
       END IF
       CALL find_string(21, '===Scalar velocity solver verbose? (true/false)', test)
       IF (test) THEN
          READ(21, *) inputs%my_par_vv_vect%verbose
       END IF
       CALL find_string(21, '===Solver type for scalar velocity (FGMRES, CG, ...)', test)
       IF (test) THEN
          READ(21, *) inputs%my_par_vv_vect%solver
       ELSE
          inputs%my_par_vv_scal%solver = inputs%my_par_vv_vect%solver
       END IF
       CALL find_string(21, '===Preconditionner type for scalar velocity solver (HYPRE, JACOBI, MUMPS...)', test)
       IF (test) THEN
          READ(21, *) inputs%my_par_vv_vect%precond
       ELSE
          inputs%my_par_vv_scal%precond = inputs%my_par_vv_vect%precond
       END IF
       CALL find_string(21, '===Scalar velocity solver strong threshold for HYPRE?', test)
       IF (test) THEN
          READ(21, *) inputs%my_par_vv_scal%strong_thr
       ELSE
    ...
    IF (inputs%if_temperature) THEN
       ...
       ALLOCATE(inputs%vol_heat_capacity(inputs%nb_dom_temp))
       ALLOCATE(inputs%temperature_diffusivity(inputs%nb_dom_temp))
