...
#include "petsc/finclude/petsc.h"
   ...
   SUBROUTINE st_aij_csr_glob_block_with_extra_layer(communicator, kmax, mesh, LA, opt_per)
      !  input coefficient structure of the matrix and
      !  perform the ordering of the unknowns
      !  jj(nodes_per_element, number_of_elements)
      !                  --->  node number in the grid
               ...
               DO nj = 1, nw
                  ...
                  DO kj = 1, kmax
                     ...
                     ELSE
                        j = LA%loc_to_glob(kj, jloc)
                     END IF
                     if (j > 872) write(*,*) '?', j ,  kj, mesh%disp(proc) - 1, mesh%domnp(proc), jloc, proc, jglob
                     IF (MINVAL(ABS(ja_work(i, 1:nja(i)) - j)) /= 0) THEN
                        nja(i) = nja(i) + 1
                        ja_work(i, nja(i)) = j
                     END IF
