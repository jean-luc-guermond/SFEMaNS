...
/**
 * @page doc_debug_test_09 Test 9: Navier-Stokes with Temperature periodic

<h3>Introduction</h3>
In this example, we check the correctness of SFEMaNS for a thermohydrodynamic problem involving periodic and Dirichlet boundary conditions.
...
p_{|t=0} &= p_0,
@f}
in the domain \f$\Omega= \{ (r,\theta,z) \in {R}^3 : (r,\theta,z) \in [0,1/2] \times [0,2\pi) \times [0,1]\} \f$
 with  \f$\Gamma= \partial \Omega\setminus\{ \{z=0\} \cup \{z=1\} \} \f$. 
We denote by \f$\textbf{e}_z\f$ the unit vector in the vertical direction.
...
The parameters are the thermal diffusivity \f$\kappa\f$, the kinetic Reynolds number \f$\Re\f$ and a thermal gravity number \f$\alpha\f$. We remind that these parameters are dimensionless.

<h3>Manufactured solutions</h3>
We approximate the following analytical solutions:
@f{align*}
T(r,\theta,z,t) & = \left(r^2\sin(2\pi z) + r^2\cos(2\pi z)(\cos(\theta)+2\sin(2\theta))\right)\cos(t),
\\ u_r(r,\theta,z,t) &=  r^3 \cos(2\pi z)\sin(t), 
\\ u_{\theta}(r,\theta,z,t) &=  r^2 \sin(2\pi z) \sin(t), 
\\ u_z(r,\theta,z,t) &= -\frac{2r^2}{\pi}\sin(2\pi z) \sin(t), 
\\ p(r,\theta,z,t) &= \frac{1}{2} \left( r^6\cos(2\pi z)^2 + r^4 \sin(2\pi z)^2 + \frac{4r^4}{\pi^2}\sin(2\pi z)^2
  \right) \sin(t)^2,
@f}
...
The initial conditions, boundary conditions and the forcing terms \f$\textbf{f}\f$ in the Navier-Stokes
 equations and \f$f_T\f$ in the temperature equation are set in the file <tt>condlim_test_9.f90</tt>.
Here is a description of the subroutines and functions of interest.
<ol>
<li>First we define the number \f$\pi\f$ at the begining of the module so that every subroutines has access to this real number.
\code
...
\endcode
where \f$t\f$ is the time.
We note that the subroutine <tt>error_petsc</tt> is here to stop the code in the case the input TYPE is different than 1 or 2 (for cosine and sine).
</ol>
...
All the other subroutines present in the file <tt>condlim_test_9.f90</tt> are not used in this test.
...
We describe the data file of this test.
 It is called <tt>debug_data_test_9</tt> and can be found in the following directory: ($SFEMaNS_DIR)/MHD_DATA_TEST_CONV_PETSC.
<ol>
...
These quantities are computed at the final time \f$t=1\f$.
 ...
 These values of reference are in the last lines of the file <tt>debug_data_test_9</tt> in the directory ($SFEMaNS_DIR)/MHD_DATA_TEST_CONV_PETSC. They are equal to:
\code
============================================
(Mesh_10_form.FEM)
===Reference results
1.864125073395549E-005  L2 error on temperature
1.211717605837686E-003  H1 error on temperature
5.566577530321795E-003  L2 norm of divergence
1.736366304283613E-003  L2 error on pressure
\endcode
...
<tr>
    <td align="center">
     @image html  fig_test_09_pre_tfin.png "Pressure in the plane plane y=0."
    </td>
    <td align="center">
    @image html  fig_test_09_vel_tfin.png  "Velocity magnitude in the plane plane y=0."
    </td>
</tr>
</table>
<table width="100%" align="center">
<tr>
    <td align="center">
     @image html  fig_test_09_temp_tfin.png "Temperature in the plane plane y=0."
    </td>
</tr>
</table>
 */
