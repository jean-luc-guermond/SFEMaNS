...
/**
 * @page doc_debug_test_02 Test 2: Navier-Stokes periodic
...
p_{|t=0} &= p_0,
@f}
in the domain \f$\Omega= \{ (r,\theta,z) \in {R}^3 : (r,\theta,z) \in [0,1/2] \times [0,2\pi) \times [0,1]\} \f$ with  \f$\Gamma= \partial \Omega\setminus\{ \{z=0\} \cup \{z=1\} \}  \f$.
The data are the source term \f$\bef\f$, the boundary data \f$\bu_{\text{bdy}}\f$,
the initial data \f$\bu_0\f$ and \f$p_0\f$. The parameter \f$\Re\f$ is the kinetic Reynolds number.
...
\\ u_{\theta}(r,\theta,z,t) &= -3 r^2 \cos(\theta) \cos(t), 
...
The initial conditions, boundary conditions and the forcing term \f$\textbf{f}\f$ in the Navier-Stokes
 equations are set in the file <tt>condlim_test_2.f90</tt>.
Here is a description of the subroutines and functions of interest.
<ol>
<li>The subroutine <tt>init_velocity_pressure_</tt> initializes the velocity field
...
<ol>
<li>First we define \f$2\pi\f$ as follows:
\code
    k = 2*ACOS(-1.d0)
\endcode
...
\code
    ...
    RETURN
\endcode
where \f$t\f$ is the time. The reader can note that the condition  "m /= 0" is not needed since we already set vv to zero when we are not dealing with the Fourier mode \f$m=1\f$.
</ol>
<li>The function <tt>pp_exact</tt> contains the analytical pressure.
 It is used to initialize the pressure.
<ol>
<li> First we define the real \f$2\pi\f$.
\code
    k = 2*ACOS(-1.d0)
\endcode
...
\code
    ...
    END IF
\endcode
<li>For the Fourier mode \f$m=1\f$, the pressure only depends of the TYPE 1 (cosine) so we write:
\code
    IF (TYPE==1) THEN
...
Thus the coefficient in sine of the presure for the Fourier mode \f$1\f$ is zero.
</ol>
<li>The function <tt>source_in_NS_momentum</tt> computes the source term \f$\bef\f$ of the Navier-Stokes equations.
</ol>
All the other subroutines present in the file <tt>condlim_test_2.f90</tt> are not used in this test.
...
We describe the data file of this test.
 It is called <tt>debug_data_test_2</tt> and can be found in the following directory: ($SFEMaNS_DIR)/MHD_DATA_TEST_CONV_PETSC.
<ol>
...
1.d0
\endcode
<li>We do not apply penalty on the divergence of the velocity field.
\code
===Coefficient for penalty of divergence in NS?
0.d0
\endcode
We note \f$0\f$ is the defaut value so this two lines are not required.
...
These quantities are computed at the final time \f$t=1\f$.
 ...
 These values of reference are in the last lines of the file <tt>debug_data_test_2</tt> in the directory($SFEMaNS_DIR)/MHD_DATA_TEST_CONV_PETSC. They are equal to:
\code
============================================
     (Mesh_10_form.FEM)
===Reference results
...
<tr>
    <td align="center">
     @image html  fig_test_02_pre_tfin.png "Pressure in the plane plane y=0."
    </td>
    <td align="center">
    @image html  fig_test_02_vel_tfin.png  "Velocity magnitude in the plane plane y=0."
    </td>
</tr>
</table>
 */
