...
/**
 * @page doc_SFEMaNS_condlim_Phiexact The function Phiexact



It is used to define the boundary condition \f$\phi_\text{bdy}\f$. It can also be used by the function <tt>init_maxwell</tt> so initial conditions match the boundary conditions.
...
<li><tt>mu_phi</tt> is the magnetic permeability in the insulating domain.
...
<h3>Exemple</h3>
Here is an exemple where we set \f$\phi_\text{bdy}= rz\f$.
...
Remark: The magnetic field has a zero divergence so \f$\GRAD \phi_\text{bdy}\f$ has to satisfy the same condition.
