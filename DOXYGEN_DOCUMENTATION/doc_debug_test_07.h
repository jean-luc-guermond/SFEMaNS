...
/**
 * @page doc_debug_test_07 Test 7: Maxwell with vacuum P2-P2
...
\f$\Omega_1= \{ (r,\theta,z) \in {R}^3 : (r,\theta,z) \in [0,1] \times [0,2\pi) \times [0,1] \; | \; 0.25\leq r^2+z^2\leq 1\} \f$
 and an insulating domain
\f$\Omega_2= \{ (r,\theta,z) \in {R}^3 : (r,\theta,z) \in [0,10] \times [0,2\pi) \times [0,10] \; | \; r^2+z^2\leq 0.25\} \cup  \{ (r,\theta,z) \in {R}^3 : (r,\theta,z) \in [0,10] \times [0,2\pi) \times [0,10] \; | \; 1\leq r^2+z^2\leq 100\} \f$.
...
@f{align*}
H_r(r,\theta,z,t) &=   3 \text{capC} (\frac{a}{\sqrt{r^2+z^2}})^3 \cos(\varphi)\sin(\varphi),
\\ H_{\theta}(r,\theta,z,t) &=0,
\\ H_z(r,\theta,z,t) &= - \text{capB} +\text{capC} (\frac{a}{\sqrt{r^2+z^2}})^3 ( 3 \cos(\varphi)^2-1) ,
\\ \phi(r,\theta,z,t) &= 
\begin{cases} \sqrt{r^2+z^2} \cos(\varphi) - \frac{\text{capD} \cos(\varphi) a^3}{r^2+z^2} \quad \text{if} \quad \sqrt{r^2+z^2}\geq 0.9,
            \\ -\text{capA} \sqrt{r^2+z^2} \cos(\varphi) \quad \text{if} \quad \sqrt{r^2+z^2}\leq 0.6,
\end{cases}
@f}
...
@f}
The source term \f$ \mathbf{j} \f$ is set to zero and the boundary data \f$\phi_{\text{bdy}}\f$ are computed accordingly.
...
<tr>
    <td align="center">
    @image html  fig_sphere_0.05_form_test_07.png  "Finite element mesh (P1) of the conducting region."
    </td>
    <td align="center">
    @image html  fig_sphere_0.05_form_vacuum_test_07.png  "Finite element mesh (P1) of the vacuum region."
    </td>
</tr>
</table>
...
The initial conditions, boundary conditions and the forcing term \f$\textbf{j}\f$ in the Maxwell
 equations are set in the file <tt>condlim_test_7.f90</tt>.
...
<li>The function <tt>Hexact</tt> contains the analytical magnetic field.
 ...
 conditions on \f$\textbf{H}x\textbf{n}\f$ with \f$\textbf{n}\f$ the outter normal vector.
<ol>
...
All the other subroutines present in the file <tt>condlim_test_7.f90</tt> are not used in this test.
...
We describe the data file of this test.
 It is called <tt>debug_data_test_7</tt> and can be found in the following directory: ($SFEMaNS_DIR)/MHD_DATA_TEST_CONV_PETSC.
<ol>
...
<li>We use one processors in the meridian section. It means the finite element mesh is not subdivised.
\code
...
2
\endcode
It means that each processors is solving the problem for \f$4/2=2\f$ Fourier modes.
...
<li>The H1 norm of the error on the scalar potential.
 ...
 These values of reference are in the last lines of the file <tt>debug_data_test_7</tt> in the directory ($SFEMaNS_DIR)/MHD_DATA_TEST_CONV_PETSC. They are equal to:
\code
============================================
...
<tr>
    <td align="center">
     @image html  fig_test_07_mag_tfin.png "Magnetic field magnitude in the plane plane y=0."
    </td>
    <td align="center">
    @image html  fig_test_07_phi_tfin.png  "Scalar potential in the plane plane y=0."
    </td>
</tr>
</table>
 */
