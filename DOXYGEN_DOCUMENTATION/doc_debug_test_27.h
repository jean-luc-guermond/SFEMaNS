...
/**
 * @page doc_debug_test_27 Test 27: Maxwell Equations with vacuum and variable magnetic permeability in (r,theta,z,t)



<h3>Introduction</h3>
In this example, we check the correctness of SFEMaNS for stationary magnetic problem involving a conducting with a variable magnetic permeability in (r,\f$\theta\f$,z,t) and an insulating domain. The magnetic permeability in the conducting region is given. We consider Dirichlet boundary conditions. We use P2 finite elements.
...
@f{align*}
\mu^c(r,\theta,z)=\frac{1}{1+f_{27}(r,z,t)\cos(m_{27}\theta)},
@f}
with \f$m_{27}=4\f$ and 
@f{align*}
f_{27}(r,z)=2^6 \frac{\text{ratio}_\mu-1}{\text{ratio}_\mu+1} \left(r(1-r)(z^2-1)  \right)^3 \cos(\omega_{27}t),
@f}
where \f$\omega_{27}=1\f$ and \f$\text{ratio}_\mu=50\f$ represents the fraction of the minimum over the maximum of the magnetic permeability in the conducting region.
...
@f{align*}
H_r(r,\theta,z,t) &= -(1+f_{27}(r,z,t)\cos(m_{27}\theta)) \cosh(z) BESSJ(1,r),
\\ H_{\theta}(r,\theta,z,t) &=0,
\\ H_z(r,\theta,z,t) &= (1+f_{27}(r,z,t)\cos(m_{27}\theta))  \sinh(z) BESSJ(0,r) ,
...
The initial conditions, boundary conditions and the forcing term \f$\textbf{j}\f$ in the Maxwell
 equations are set in the file <tt>condlim_test_27.f90</tt>. This condlim also uses functions of the file <tt>test_27.f90</tt> to define the function \f$f_{27}\f$, the magnetic permeability and its gradient. Here is a description of the subroutines and functions of interest of the file <tt>condlim_test_27.f90</tt>.
<ol>
...
\code
    IF (m==0) THEN
       ...
       END IF
\endcode
<li>If the Fourier mode is equal to \f$m_{27}\f$, it is defined as follows:
\code
    ELSE IF (m==mode_mu_T27) THEN
       ...
       END IF
\endcode
where f_test_wtime_T27 represents the function \f$f_{27}\f$. It is defined in the file <tt>test_27.f90</tt>.
...
<ol>
<li>If the Fourier mode m is not equal to \f$m_{27}\f$, we set the source term to zero.
\code
...
\code
   ...
   z = rr(2)
\endcode
<li>For the Fourier mode \f$m=m_{27}\f$, we define the source term depending of its TYPE (1 and 2 for the component radial cosine and sine, 3 and 4 for the component azimuthal cosine and sine, 5 and 6 for the component vertical cosine and sine) as follows:
\code
...
\code
    ...
    RETURN
\endcode
<li>The function <tt>grad_mu_bar_in_fourier_space</tt> defines the gradient of mu_bar. It is done by using the function <tt>grad_mu_bar_in_fourier_space_anal_T27</tt> of the file <tt>test_27.f90</tt>.
\code
    vv=grad_mu_bar_in_fourier_space_anal_T27(pt,pt_id) 
    RETURN
\endcode
<li>The function <tt>mu_in_real_space</tt> defines the magnetic permeability. It is done by using the function <tt>mu_in_real_space_anal_T27</tt> of the file <tt>test_27.f90</tt>.
\code
    vv = mu_in_real_space_anal_T27(H_mesh,angles,nb_angles,nb,ne)
    RETURN
\endcode
</ol>
All the other subroutines present in the file <tt>condlim_test_27.f90</tt> are not used in this test.  We remind that the bessel function \f$BESSJ0\f$ and \f$BESSJ1\f$ are defined in the file <tt>bessel.f90</tt> in the following directory: ($SFEMaNS_DIR)/FEMSUB.
We refer to the section \ref doc_SFEMaNS_condlim for a description of all the subroutines of the condlim file. 
 The following describes the functions of interest of the file <tt>test_27.f90</tt>.
<ol>
<li>First, we define the real numbers \f$\text{ratio}_\mu\f$, \f$b\f$,\f$\omega_{27}\f$ and the integer \f$m_{27}\f$.
\code
...
\endcode
<li>The function <tt>f_test_wtime_T27</tt> defines the function \f$f_{27}\f$.
\code
    vv = b_factor_T27*COS(omega_T27*t)*(r*(1-r)*(z**2-1))**3
    RETURN
\endcode
<li>The function <tt>dfdr_test_wtime_T27</tt> defines the r-derivative of \f$f_{27}\f$.
\code
    vv = 3 * b_factor_T27 * COS(omega_T27*t) * (z**2-1)**3 * (r*(1-r))**2 * (1-2*r)
    RETURN
\endcode
<li>The function <tt>dfdz_test_wtime_T27</tt> defines the r-derivative of \f$f_{27}\f$.
\code
    vv = 3*b_factor_T27*COS(omega_T27*t) *(r*(1-r))**3*(z**2-1)**2*(2*z)
    RETURN
\endcode
<li>The function <tt>f_test_T27</tt> defines a function \f$\overline{f}_{27}\f$ larger than \f$f_{27}\f$ that only depends of (r,z).
\code
    vv = b_factor_T27*(r*(1-r)*(z**2-1))**3
    RETURN
\endcode
<li>The function <tt>dfdr_test_T27</tt> defines the r-derivative of \f$\overline{f}_{27}\f$.
\code
    vv = 3 * b_factor_T27 * (z**2-1)**3 * (r*(1-r))**2 * (1-2*r)
    RETURN
\endcode
<li>The function <tt>dfdz_test_T27</tt> defines the z-derivative of \f$\overline{f}_{27}\f$.
\code
    vv = 3*b_factor_T27*(r*(1-r))**3*(z**2-1)**2*(2*z)
    RETURN
\endcode
<li>The function <tt>mu_bar_in_fourier_space_anal_T27</tt> defines stabilization term mu_bar such that it is smaller than mu everywhere in the domain. 
\code
    ...
    RETURN
\endcode
<li>The function <tt>grad_mu_bar_in_fourier_space_anal_T27</tt> defines the gradient of mu_bar on gauss points.
\code
    r(1)=pt(1)
    z(1)=pt(2)
    tmp=f_test_T27(r,z)
    ...
    RETURN
\endcode
<li>The function <tt>mu_in_real_space_anal_T27</tt> defines the magnetic permeability (depending of the node in the meridian plan and its angle).
\code
    DO ang = 1, nb_angles
...
We describe the data file of this test.
 It is called <tt>debug_data_test_27</tt> and can be found in the following directory: ($SFEMaNS_DIR)/MHD_DATA_TEST_CONV_PETSC.
<ol>
...
0
\endcode
<li>The magnetic permeability is defined with a function of the file <tt>condlim_test_27.f90</tt>.
\code
===Is permeability defined analytically (true/false)?
.t.
\endcode
<li>We construct a stablization term, denoted mu_bar, on the gauss points by a finite element interpolation of its value on the nodes. This stabilization term is defined in the file <tt>condlim_test_27.f90</tt>.
\code
...
<li>The L2 norm of the error on the scalar potential.
 ...
 These values of reference are in the last lines of the file <tt>debug_data_test_27</tt> in the directory ($SFEMaNS_DIR)/MHD_DATA_TEST_CONV_PETSC. They are equal to:
\code
============================================
...
<tr>
    <td align="center">
     @image html  fig_test_27_mag_tfin.png "Magnetic field magnitude in the plane plane y=0."
    </td>
    <td align="center">
    @image html  fig_test_27_phi_tfin.png  "Scalar potential in the plane plane y=0."
    </td>
</tr>
</table>
