...
/**
 * @page doc_debug_test_34 Test 34: Ferrohydrodynamic periodic

<h3>Introduction</h3> In this example, we check the correctness of
SFEMaNS for a ferrohydrodynamic fluid problem periodic in the \f$z\f$
direction. The Maxwell, the Navier-Stokes and the energy equations are
solved. The domain is composed of a solid subdomain and a fluid
subdomain. The Maxwell equations are reduced to the magnetoststic
equations due to the quasi-static approximation for the magnetic
field. The fluid and solid regions do not have the same thermal
properties: the volumetric heat capacity and the thermal conductivity
are different. This case is useful to study the cooling of a solid by
a ferrofluid by convection and magnetoconvection. Periodicity in the
\f$z\f$ direction is used. The other difference with Test 33 is that
the pressure is non zero now.

The domain of computation is \f$\Omega= \{ (r,\theta,z) \in {R}^3 : (r,\theta,z) \in (0,1) \times [0,2\pi) \times (0,1)\} \f$. Notice \f$\Gamma= \partial \Omega\f$. It is composed of a solid and a fluid subdomain:
\f{align*}{
...
The initial conditions, boundary conditions and the forcing terms are set in the file <tt>condlim_test_34.f90</tt>.
...
\code
    REAL(KIND=8), DIMENSION(SIZE(rr,2))                  :: vv, r, z, lambda
\endcode
<li>The coefficient \f$\alpha\f$ of the Boussinesq force is declared and the limit between fluid and solid region and \f$\pi\f$ are defined:
\code
...
All the other subroutines present in the file <tt>condlim_test_34.f90</tt> are not used in this test.
...
We describe the data file of this test.
 It is called <tt>debug_data_test_34</tt> and can be found in the directory ($SFEMaNS_DIR)/MHD_DATA_TEST_CONV_PETSC.
<ol>
...
These quantities are computed at the final time \f$t=1\f$.
 ...
 These values of reference are in the last lines of the file <tt>debug_data_test_34</tt> in ($SFEMaNS_DIR)/MHD_DATA_TEST_CONV_PETSC. They are equal to:
\code
============================================
(SOLID_FLUID_10.FEM)
===Reference results
3.6356759863036140E-004  L2-norm of error on u / L2-norm of u exact
6.1668032938189378E-002  L2-norm of error on p / L2-norm of p exact
2.1997806414233940E-004  L2-norm of error on T / L2-norm of T exact
2.8695295848809800E-003  L2-norm of error on H / L2-norm of H exact
\endcode
...
<tr>
    <td align="center">
     @image html  fig_test_34_pre_tfin.png "Pressure in the plane y=0."
    </td>
</tr>
</table>
<table width="100%" align="center">
<tr>
    <td align="center">
    @image html  fig_test_34_vel_tfin.png  "Velocity magnitude in the plane y=0."
    </td>
</tr>
</table>
<table width="100%" align="center">
<tr>
    <td align="center">
     @image html  fig_test_34_temp_tfin.png "Temperature in the plane y=0."
    </td>
</tr>
</table>
<table width="100%" align="center">
<tr>
    <td align="center">
     @image html  fig_test_34_mag_tfin.png "Magnetic field magnitude in the plane y=0."
    </td>
</tr>
</table>
 */
