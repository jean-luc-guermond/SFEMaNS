...
/**
 * @page doc_debug_test_05 Test 5: Maxwell with vacuum P1-P2


<h3>Introduction</h3>
In this example, we check the correctness of SFEMaNS for a magnetic problem involving a conducting and an insulating domain. We consider Dirichlet boundary conditions. We use P1 finite elements for the magnetic field and P2 finite elements for the scalar potential.
...
\end{cases}
\f}
in the domain \f$\Omega= \{ (r,\theta,z) \in {R}^3 : (r,\theta,z) \in [0,10] \times [0,2\pi) \times [-2,2]\} \f$.
 This domain is the union of a conducting domain
\f$\Omega_1= \{ (r,\theta,z) \in {R}^3 : (r,\theta,z) \in [0,2] \times [0,2\pi) \times [-2,2]\} \f$
 and an insulating domain  
\f$\Omega_2= \{ (r,\theta,z) \in {R}^3 : (r,\theta,z) \in [2,10] \times [0,2\pi) \times [-2,2]\} \f$.
We also set \f$\Sigma= \Omega_1 \cap \Omega_2 \f$ and \f$\Gamma_1= \partial \Omega_1\setminus \Sigma \f$ 
...
@f{align*}
H_r(r,\theta,z,t) &= \frac{\alpha z}{\mu^c} \left( \cos(\theta) + \frac{r}{4}\cos(2\theta) \right)\cos(t)
+ \frac{\beta z}{\mu^c} \left(\sin(\theta) + \frac{r}{4}\sin(2\theta)  \right)\cos(t),
\\ H_{\theta}(r,\theta,z,t) &= \frac{\beta z}{\mu^c}  \left( \cos(\theta) + \frac{r}{4}\cos(2\theta) \right)\cos(t)
-\frac{\alpha z}{\mu^c} \left(\sin(\theta) + \frac{r}{4}\sin(2\theta) \right)\cos(t),
\\ H_z(r,\theta,z,t) &= \frac{\alpha}{\mu^c} \left(r\cos(\theta) +\frac{r^2}{8}\cos(2\theta)  \right) \cos(t)
+ \frac{\beta}{\mu^c} \left(r\sin(\theta) +\frac{r^2}{8}\sin(2\theta)  \right) \cos(t),
\\ \phi(r,\theta,z,t) &= \frac{\alpha z}{\mu^v} \left( r \cos(\theta) + \frac{r^2}{8} \cos(2\theta) \right) \cos(t) 
+ \frac{\beta z}{\mu^v} \left( r \sin(\theta) + \frac{r^2}{8} \sin(2\theta) \right) \cos(t),
@f}
where \f$\alpha\f$ and \f$\beta\f$ are parameters. We also set the given velocity field as follows:
@f{align*}
u^\text{given}_r(r,\theta,z,t) &= \alpha z \left(\cos(\theta) + \frac{r}{4}\cos(2\theta) \right)
+ \beta z \left( \sin(\theta) + \frac{r}{4}\sin(2\theta)  \right), 
\\ u^\text{given}_{\theta}(r,\theta,z,t) &= \beta z \left( \cos(\theta) + \frac{r}{4}\cos(2\theta) \right)
-\alpha z \left( \sin(\theta) + \frac{r}{4}\sin(2\theta)  \right), 
\\ u^\text{given}_z(r,\theta,z,t) &= \alpha \left( r \cos(\theta) + \frac{r^2}{8}\cos(2\theta) \right)
+\beta \left( r \sin(\theta) + \frac{r^2}{8}\sin(2\theta) \right).
@f}
...
The finite element mesh used for this test is named <tt>CYL10_TCM_PERIO_form.FEM</tt>.
 The mesh size for the P1 approximation is \f$0.1\f$ for \f$r\leq2\f$ and \f$0.5\f$ for \f$r=10\f$.
 You can generate this mesh with the files in the following directory:
 ($SFEMaNS_MESH_GEN_DIR)/EXAMPLES/EXAMPLES_MANUFACTURED_SOLUTIONS/CYL10_TCM_PERIO_form.
The following images show the mesh for P1 finite elements for the conducting and vacuum regions.

<table width="70%" align="center">
<tr>
    <td align="center">
    @image html  fig_CYL10_TCM_PERIO_form.png  "Finite element mesh (P1) of the conducting region."
    </td>
    <td align="center">
    @image html  fig_CYL10_TCM_PERIO_form_vacuum.png  "Finite element mesh (P1) of the vacuum region."
    </td>
</tr>
</table>
...
The initial conditions, boundary conditions and the forcing term \f$\textbf{j}\f$ in the Maxwell
 equations are set in the file <tt>condlim_test_5.f90</tt>.
...
\code
    ...
    END IF
\endcode
<li>For the Fourier modes \f$m\in\{1,2\}\f$, we define the magnetic field depending of its TYPE (1 and 2 for the component radial cosine and sine, 3 and 4 for the component azimuthal cosine and sine, 5 and 6 for the component vertical cosine and sine) as follows:
\code
    IF (TYPE == 1) THEN
       vv = alpha*z*(r**(m-1))*m
    ELSEIF (TYPE == 2) THEN
...
\code
    IF (m == 0) RETURN
\endcode
<li>For the Fourier modes \f$m\in\{1,2\}\f$, we define it depending of its TYPE (1 and 2 for the component radial cosine and sine, 3 and 4 for the component azimuthal cosine and sine, 5 and 6 for the component vertical cosine and sine).
\code
    IF  (TYPE == 1) THEN
       vv = 0.d0
    ELSEIF (TYPE == 2) THEN
...
All the other subroutines present in the file <tt>condlim_test_5.f90</tt> are not used in this test.
...
We describe the data file of this test.
 It is called <tt>debug_data_test_5</tt> and can be found in the following directory: ($SFEMaNS_DIR)/MHD_DATA_TEST_CONV_PETSC.
<ol>
...
<li>The H1 norm of the error on the scalar potential.
 ...
 These values of reference are in the last lines of the file <tt>debug_data_test_5</tt> in the directory ($SFEMaNS_DIR)/MHD_DATA_TEST_CONV_PETSC. They are equal to:
\code
============================================
CYL10_TCM_PERIO_form.FEM, P1P2
===Reference results
1.89514002590318153E-004 L2 error on Hn
5.76310491335821329E-004 L2 error on Curl(Hn)
3.22708393165986162E-003 L2 norm of Div(mu Hn)
5.29871541028787640E-006 H1 error on phin
\endcode
...
<tr>
    <td align="center">
     @image html  fig_test_05_mag_tfin.png "Magnetic field magnitude in the plane plane y=0."
    </td>
</tr>
<tr>
    <td align="center">
    @image html  fig_test_05_phi_tfin.png  "Scalar potential in the plane plane y=0."
    </td>
</tr>
</table>
 */
