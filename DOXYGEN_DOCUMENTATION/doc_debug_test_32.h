...
/**
 * @page doc_debug_test_32 Test 32: Navier-Stokes with Temperature, temperature computed in a solid and a fluid region, periodic manufactured solution and non null pressure, restart from test 31 with interpolation

<h3>Introduction</h3>
In this example, we check the correctness of the module for interpolation on a thermohydrodynamic problem. The simulation is the next part of test 31 (\f$1\f$-periodic in \f$z\f$ manufactured solution). Between test 31 and 32, the mesh is changed and so is the number of processors in the meridian section (1 proc. to 5 proc.). An interpolation step takes place during the debug. 
...
The finite element mesh used for this test is named <tt>SOLID_FLUID_20.FEM</tt> and 
 has a mesh size of \f$0.05\f$ for the P1 approximation. 
You can generate this mesh with the files in the following directory:
 ($SFEMaNS_MESH_GEN_DIR)/EXAMPLES/EXAMPLES_MANUFACTURED_SOLUTIONS/SOLID_FLUID_20.
The following image shows the mesh for P1 finite elements.
<table width="100%" align="center">
<tr>
    <td align="center">
    @image html  fig_SOLID_FLUID_20.png  "Finite element mesh (P1)."
    </td>
</tr>
</table>
...
The initial conditions, boundary conditions and the forcing terms \f$\textbf{f}\f$ in the Navier-Stokes
 equations and \f$f_T\f$ in the temperature equations are set in the file <tt>condlim_test_32.f90</tt>, which is identical to <tt>condlim_test_31.f90</tt>.
...
All the other subroutines present in the file <tt>condlim_test_32.f90</tt> are not used in this test.
We refer to the section \ref doc_SFEMaNS_condlim for a description of all the subroutines of the condlim file.

<h3>Setting in the interpolation data file</h3>

We describe the interpolation data file needed to interpolate the fields of velocity, pressure and temperature at the end of test 31 on the new mesh. It is called <tt>debug_data_test_31_interpol</tt> and can be found in the directory ($SFEMaNS_DIR)/MHD_DATA_TEST_CONV_PETSC.
<ol>
<li>We indicate informations about the input simulation.
<ol>
<li>We indicate the number of processors in the meridian section.
\code
===Number of processors in meridian section (Input)
1
\endcode
<li>We indicate the problem type and wether there is a temperature field.
\code
...
.t.
\endcode
<li>We indicate the mesh informations.
\code
...
<li>We indicate informations about the output simulation.
<ol>
<li>We indicate the number of processors in the meridian section.
\code
...
<li>We detail the process of interpolation.
<ol>
<li>We indicate wether the mesh changes.
\code
===Should data be interpolated on new mesh? (True/False)
.t.
\endcode
It could not be the case if the user only wanted to change the number of processors in the meridian section. The module for mesh interpolation must be used in this case too.
<li>We indicate the number of time steps we want to interpolate. We are only interested in the last time step here.
\code
===How many files (times steps) should be converted?
1
\endcode
<li>We set the starting index. Restart files are indexed with a number. We have written only one restart file (for each field). We select 1.
\code
===What is the starting index? (suite*_I*index.mesh*), index is an integer in [1,999]
1
\endcode
<li>We indicate the number of Fourier modes.
\code
===Number of Fourier modes
3
\endcode
</ol>
<li>We set the periodic boundaries (for output simulation).
<ol>
...
<li>We set the number of domains and their label, see the files associated to the generation of the mesh,
 where the code approximates the Navier-Stokes equations (for output simulation).
\code
...
\endcode
<li>We set the number of domains and their label, see the files associated to the generation of the mesh,
 where the code approximates the temperature equation (for output simulation).
\code
...
1 2
\endcode
<li>We set the interfaces between regions where only the temperature is solved and regions where velocity and tenperature is solved (for output simulation). It is necessary to impose boundary conditions on the velocity at these interfaces.
\code
...
We describe the data file of this test. It is called <tt>debug_data_test_32</tt> and can be found in the directory ($SFEMaNS_DIR)/MHD_DATA_TEST_CONV_PETSC.
<ol>
...
<li>We use five processors in the meridian section.
\code
...
1
\endcode
It means that each processors is solving the problem for the \f$3\f$ Fourier modes.
<li>We approximate the Navier-Stokes equations by setting:
\code
===Problem type: (nst, mxw, mhd, fhd)
'nst'
\endcode
<li>We restart the computations from previous results.
\code
...
.t.
\endcode
The computation starts from the time \f$t=1\f$ (end of test 31).
<li>We use a time step of \f$0.005\f$ and solve the problem over \f$200\f$ time iterations.
\code
===Time step and number of time iterations
5.d-3 200
\endcode
<li>We read the metis partition provided by the mesh interpolation module.
\code
...
These quantities are computed at the final time \f$t=1\f$.
 ...
 These values of reference are in the last lines of the file <tt>debug_data_test_32</tt> in ($SFEMaNS_DIR)/MHD_DATA_TEST_CONV_PETSC. They are equal to:
\code
============================================
(SOLID_FLUID_20.FEM)
===Reference results
6.06639948859780695E-005  L2-norm of error on u / L2-norm of u exact
7.38878983488354922E-003  L2-norm of error on p / L2-norm of p exact
2.45969800747957744E-005  L2-norm of error on T / L2-norm of T exact
2.77731794127341280E-004  H1-norm of error on T / H1-norm of T exact
\endcode
...
<tr>
    <td align="center">
     @image html  fig_test_32_pre_tfin.png "Pressure in the plane plane y=0."
    </td>
</tr>
</table>
<table width="100%" align="center">
<tr>
    <td align="center">
    @image html  fig_test_32_vel_tfin.png  "Velocity magnitude in the plane y=0."
    </td>
</tr>
</table>
<table width="100%" align="center">
<tr>
    <td align="center">
     @image html  fig_test_32_temp_tfin.png "Temperature in the plane y=0."
    </td>
</tr>
</table>
 */
