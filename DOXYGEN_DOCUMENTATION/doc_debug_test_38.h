...
The initial conditions, boundary conditions and the forcing terms are set in the file <tt>condlim_test_38.f90</tt>.
...
\code
    REAL(KIND=8)                                      :: r0 = 0.5d0, pi = acos(-1.d0)
\endcode
<li>The coefficient \f$\beta\f$ is defined based on the data file.
\code
...
\code
    ...
    END DO
\endcode
<li>The source term \f$ f_T = c\partial_t T+ c\tilde{\bu} \cdot \GRAD T + T \frac{\partial \chi}{\partial T}(T) \left( \partial_t \left( \frac{H^2}{2} \right) + \tilde{\bu} \SCAL \GRAD \left( \frac{H^2}{2} \right) \right) - \DIV(\lambda\GRAD T) \f$ is defined in four parts. Firstly, we define the part \f$ c\partial_t T - \DIV (\lambda\GRAD T) \f$:
\code
    IF (TYPE==1) THEN
       IF (m==0) THEN
...
\code
    ...
    END IF
\endcode
We then add the part \f$ T \frac{\partial \chi}{\partial T}(T) \partial_t \left( \frac{H^2}{2} \right) \f$. Again, it is different from 0 only in the fluid:
\code
    IF (TYPE==1) THEN
       IF (m==0) THEN
         DO i=1,size(rr,2)
             IF (r(i)>r0) THEN
                vv(i) = vv(i) - beta*(r(i)**8*(r(i)-r0)**2*cos(t)**3*(-215.d0-136*pi**2*r(i)**2+&
                     (-215.d0+136*pi**2*r(i)**2)*cos(4*pi*z(i)))*sin(t)*sin(2*pi*z(i))**2)/&
                     (8*lambda(i))
             END IF
          END DO
       ELSE IF (m==1) THEN
         DO i=1,size(rr,2)
             IF (r(i)>r0) THEN
                vv(i) = vv(i) - beta*(5*r(i)**8*(r(i)-r0)**2*cos(t)**3*(-11.d0-8*pi**2*r(i)**2+&
                     (-11.d0+8*pi**2*r(i)**2)*cos(4*pi*z(i)))*sin(t)*sin(2*pi*z(i))**2)/&
                     (2*lambda(i))
             END IF
          END DO
       ELSE IF (m==2) THEN
         DO i=1,size(rr,2)
             IF (r(i)>r0) THEN
                vv(i) = vv(i) - beta*(7*r(i)**8*(r(i)-r0)**2*cos(t)**3*sin(t)*sin(4*pi*z(i))**2)/&
                     (2*lambda(i))
             END IF
          END DO
       ELSE IF (m==3) THEN
         DO i=1,size(rr,2)
             IF (r(i)>r0) THEN
                vv(i) = vv(i) + beta*(r(i)**8*(r(i)-r0)**2*cos(t)**3*(-19.d0-8*pi**2*r(i)**2+&
                     (-19.d0+8*pi**2*r(i)**2)*cos(4*pi*z(i)))*sin(t)*sin(2*pi*z(i))**2)/&
                     (2*lambda(i))
             END IF
          END DO
       ELSE IF (m==4) THEN
         DO i=1,size(rr,2)
             IF (r(i)>r0) THEN
                vv(i) = vv(i) + beta*(r(i)**8*(r(i)-r0)**2*cos(t)**3*(-15.d0-8*pi**2*r(i)**2+&
                     (-15.d0+8*pi**2*r(i)**2)*cos(4*pi*z(i)))*sin(t)*sin(2*pi*z(i))**2)/&
                     (8*lambda(i)**2)
             END IF
          END DO
       END IF
    ELSE IF (TYPE==2) THEN
       IF (m==1) THEN
         DO i=1,size(rr,2)
             IF (r(i)>r0) THEN
                vv(i) = vv(i) - beta*(4*r(i)**8*(r(i)-r0)**2*cos(t)**3*(-9.d0-5*pi**2*r(i)**2+&
                     (-9.d0+5*pi**2*r(i)**2)*cos(4*pi*z(i)))*sin(t)*sin(2*pi*z(i))**2)/&
                     lambda(i)**2
             END IF
          END DO
       ELSE IF (m==2) THEN
         DO i=1,size(rr,2)
             IF (r(i)>r0) THEN
                vv(i) = vv(i) - beta*(2*r(i)**8*(r(i)-r0)**2*cos(t)**3*(-13.d0-8*pi**2*r(i)**2+&
                     (-13.d0+8*pi**2*r(i)**2)*cos(4*pi*z(i)))*sin(t)*sin(2*pi*z(i))**2)/&
                     lambda(i)**2
             END IF
          END DO
       ELSE IF (m==3) THEN
         DO i=1,size(rr,2)
             IF (r(i)>r0) THEN
                vv(i) = vv(i) - beta*(4*r(i)**8*(r(i)-r0)**2*cos(t)**3*(-1.d0-pi**2*r(i)**2+&
                     (-1.d0+pi**2*r(i)**2)*cos(4*pi*z(i)))*sin(t)*sin(2*pi*z(i))**2)/&
                     lambda(i)**2
             END IF
          END DO
       ELSE IF (m==4) THEN
         DO i=1,size(rr,2)
             IF (r(i)>r0) THEN
                vv(i) = vv(i) - beta*(r(i)**8*(r(i)-r0)**2*cos(t)**3*sin(t)*sin(4*pi*z(i))**2)/&
                     (2*lambda(i)**2)
             END IF
          END DO
       END IF
    END IF
\endcode
We finally add the part \f$ T \frac{\partial \chi}{\partial T}(T) \tilde{\bu} \SCAL \GRAD \left( \frac{H^2}{2} \right) \f$. Again, it is different from 0 only in the fluid:
\code
    IF (TYPE==1) THEN
       IF (m==0) THEN
         DO i=1,size(rr,2)
             IF (r(i)>r0) THEN
                vv(i) = vv(i) + beta*(pi*r(i)**7*(r(i)-r0)**3*cos(t)**5*cos(2*pi*z(i))*&
                     (-903*r0+r(i)*(1553.d0-8*pi**2*r(i)*(25*r(i)+29*r0))+&
                     (35*r0+r(i)*(-685.d0+8*pi**2*r(i)*(25*r(i)+29*r0)))*cos(4*pi*z(i)))*sin(2*pi*z(i))**2)/&
                     (4*lambda(i)**2)
             END IF
          END DO
       ELSE IF (m==1) THEN
         DO i=1,size(rr,2)
             IF (r(i)>r0) THEN
                vv(i) = vv(i) + beta*(pi*r(i)**7*(r(i)-r0)**3*cos(t)**5*cos(2*pi*z(i))*&
                     (-973*r0+r(i)*(1787.d0-16*pi**2*r(i)*(17*r(i)+20*r0))+&
                     (49*r0+r(i)*(-863.d0+16*pi**2*r(i)*(17*r(i)+20*r0)))*cos(4*pi*z(i)))*sin(2*pi*z(i))**2)/&
                     (4*lambda(i)**2)
             END IF
          END DO
       ELSE IF (m==2) THEN
         DO i=1,size(rr,2)
             IF (r(i)>r0) THEN
                vv(i) = vv(i) + beta*(8*pi*r(i)**7*(r(i)-r0)**3*cos(t)**5*cos(2*pi*z(i))*&
                     (8*r0-r(i)*(7.d0+2*pi**2*r(i)*(r(i)+r0))+(r0+2*r(i)*(-1.d0+pi**2*r(i)*(r(i)+r0)))*&
                     cos(4*pi*z(i)))*sin(2*pi*z(i))**2)/&
                     lambda(i)**2
             END IF
          END DO
       ELSE IF (m==3) THEN
         DO i=1,size(rr,2)
             IF (r(i)>r0) THEN
                vv(i) = vv(i) - beta*(pi*r(i)**7*(r(i)-r0)**3*cos(t)**5*cos(2*pi*z(i))*&
                     (-957*r0+r(i)*(1403.d0-16*pi**2*r(i)*(2*r(i)+7*r0))+&
                     (-79*r0+r(i)*(-367.d0+16*pi**2*r(i)*(2*r(i)+7*r0)))*cos(4*pi*z(i)))*sin(2*pi*z(i))**2)/&
                     (8*lambda(i)**2)
             END IF
          END DO
       ELSE IF (m==4) THEN
         DO i=1,size(rr,2)
             IF (r(i)>r0) THEN
                vv(i) = vv(i) - beta*(pi*r(i)**7*(r(i)-r0)**3*cos(t)**5*cos(2*pi*z(i))*&
                     (-167*r0+r(i)*(273.d0-8*pi**2*r(i)*(r(i)+5*r0))+&
                     (-29*r0+r(i)*(-77.d0+8*pi**2*r(i)*(r(i)+5*r0)))*cos(4*pi*z(i)))*sin(2*pi*z(i))**2)/&
                     (4*lambda(i)**2)
             END IF
          END DO
       ELSE IF (m==5) THEN
         DO i=1,size(rr,2)
             IF (r(i)>r0) THEN
                vv(i) = vv(i) - beta*(pi*r(i)**7*(r(i)-r0)**3*cos(t)**5*cos(2*pi*z(i))*&
                     (59*r(i)-29*r0-16*pi**2*r(i)**2*r0+(-15*r(i)-15*r0+16*pi**2*r(i)**2*r0)*&
                     cos(4*pi*z(i)))*sin(2*pi*z(i))**2)/&
                     (8*lambda(i)**2)
             END IF
          END DO
    ...
    ELSE IF (TYPE==2) THEN
       IF (m==1) THEN
         DO i=1,size(rr,2)
             IF (r(i)>r0) THEN
                vv(i) = vv(i)+ beta*(pi*r(i)**7*(r(i)-r0)**3*cos(t)**5*cos(2*pi*z(i))*&
                     (-629*r0+r(i)*(1021.d0-32*pi**2*r(i)*(3*r(i)+4*r0))+&
                     2*(3*r0+r(i)*(-199.d0+16*pi**2*r(i)*(3*r(i)+4*r0)))*cos(4*pi*z(i)))*sin(2*pi*z(i))**2)/&
                     (2*lambda(i)**2)
             END IF
          END DO
       ELSE IF (m==2) THEN
         DO i=1,size(rr,2)
             IF (r(i)>r0) THEN
                vv(i) = vv(i) + beta*(pi*r(i)**7*(r(i)-r0)**3*cos(t)**5*cos(2*pi*z(i))*&
                     (-523*r0+r(i)*(891.d0-8*pi**2*r(i)*(11*r(i)+17*r0))+&
                     (-7*r0+r(i)*(-361.d0+8*pi**2*r(i)*(11*r(i)+17*r0)))*cos(4*pi*z(i)))*sin(2*pi*z(i))**2)/&
                     (2*lambda(i)**2)
             END IF
          END DO
       ELSE IF (m==3) THEN
         DO i=1,size(rr,2)
             IF (r(i)>r0) THEN
                vv(i) = vv(i) + beta*(pi*r(i)**7*(r(i)-r0)**3*cos(t)**5*cos(2*pi*z(i))*&
                     (-239.d0*r0+r(i)*(503.d0-64*pi**2*r(i)*(r(i)+2*r0))+&
                     8*(-2*r0+r(i)*(-31.d0+8*pi**2*r(i)*(r(i)+2*r0)))*cos(4*pi*z(i)))*sin(2*pi*z(i))**2)/&
                     (4*lambda(i)**2)
             END IF
          END DO
       ELSE IF (m==4) THEN
         DO i=1,size(rr,2)
             IF (r(i)>r0) THEN
                vv(i) = vv(i) + beta*(pi*r(i)**7*(r(i)-r0)**3*cos(t)**5*cos(2*pi*z(i))*&
                     (63*r0-r(i)*(47.d0+8*pi**2*r(i)*(r(i)+3*r0))+&
                     (3*r0+r(i)*(-19.d0+8*pi**2*r(i)*(r(i)+3*r0)))*cos(4*pi*z(i)))*sin(2*pi*z(i))**2)/&
                     (4*lambda(i)**2) 
             END IF
          END DO
       ELSE IF (m==5) THEN
         DO i=1,size(rr,2)
             IF (r(i)>r0) THEN
                vv(i) = vv(i) + beta*(pi*r(i)**7*(r(i)-r0)**3*cos(t)**5*cos(2*pi*z(i))*&
                     (-35*r(i)+27*r0+4*(r(i)+r0)*cos(4*pi*z(i)))*sin(2*pi*z(i))**2)/&
                     (4*lambda(i)**2)
             END IF
          END DO
       END IF
    END IF
\endcode
In the second, third and fourth part, the array is filled cell by cell because we have to test if the node is in the fluid region.
</ol>
...
<li>The part of the source term \f$ \partial_t\bu+\left(\ROT\bu\right)\CROSS\bu - \frac{1}{\Re}\LAP \bu +\GRAD p\f$ is then added.
\code
    IF (TYPE==1) THEN
       IF (mode==0) THEN
    ...
    END IF
\endcode
<li>The last part of the source term  \f$ - \chi(T) \GRAD \left( \frac{H^2}{2} \right) \f$ is then added.
\code
    IF (TYPE == 1) THEN
       IF (mode == 0) THEN
          vv = vv + beta * r**7 * (r - r0)**2 * cos(time)**4 * (215.d0 + 204 * pi**2 * r**2 + &
               (215.d0 - 204 * pi**2 * r**2) * cos(4*pi * z)) * sin(2*pi*z)**2 / &
               (8 * lambda**2)
       ELSE IF (mode == 1) THEN
          vv = vv - beta * 5 * r**7 * (r - r0)**2 * cos(time)**4 * (-11.d0 - 12 * pi**2 * r**2 + &
               (-11.d0 + 12 * pi**2 * r**2) * cos(4*pi*z)) * sin(2*pi*z)**2 / &
               (2 * lambda**2) 
       ELSE IF (mode == 2) THEN
          vv = vv - beta * 7 * r**7 * (r - r0)**2 * cos(time)**4 * sin(4*pi*z)**2 / &
               (2 * lambda**2)
       ELSE IF (mode == 3) THEN
          vv = vv + beta * r**7 * (r - r0)**2 * cos(time)**4 * (-19.d0 - 12 * pi**2 *r**2 + &
               (-19.d0 + 12 * pi**2 *r**2) * cos(4*pi*z)) * sin(2*pi*z)**2 / &
               (2 * lambda**2)
       ELSE IF (mode == 4) THEN
          vv = vv + beta * 3 * r**7 * (r - r0)**2 * cos(time)**4 * (-5.d0 - 4 * pi**2 * r**2 + &
               (-5.d0 + 4 * pi**2 * r**2) * cos(4*pi*z)) * sin(2*pi*z)**2 / &
               (8 * lambda**2)
       END IF
    ELSE IF (TYPE == 2) THEN
       IF (mode == 1) THEN
          vv = vv - beta * 6 * r**7 * (r - r0)**2 * cos(time)**4 * (-6.d0 - 5 * pi**2 * r**2 + &
               (-6.d0 + 5.d0 * pi**2 * r**2) * cos(4*pi*z)) * sin(2*pi*z)**2 / &
               lambda**2
       ELSE IF (mode == 2) THEN
          vv = vv + beta * 2 * r**7 * (r - r0)**2 * cos(time)**4 * (13.d0 + 12 * pi**2 * r**2 + &
               (13.d0 - 12 * pi**2 * r**2) * cos(4*pi*z)) * sin(2*pi*z)**2 / &
               lambda**2
       ELSE IF (mode == 3) THEN
          vv = vv + beta * 2 * r**7 * (r - r0)**2 * cos(time)**4 * (2.d0 + 3 * pi**2 * r**2 + &
               (2.d0 - 3 * pi**2 * r**2) * cos(4*pi*z)) * sin(2*pi*z)**2 / &
               lambda**2
       ELSE IF (mode == 4) THEN
          vv = vv - beta * r**7 * (r - r0)**2 * cos(time)**4 * sin(4*pi*z)**2 / &
               (2 * lambda**2)
       END IF
    ELSE IF (TYPE == 3) THEN
       IF (mode == 0) THEN
          vv = vv + beta * r**7 * (r - r0)**2 * cos(time)**4 * (15.d0 + 8 * pi**2 * r**2 + &
               (15.d0 - 8 * pi**2 * r**2) * cos(4*pi*z)) * sin(2*pi*z)**2 / &
               (2 * lambda**2)
       ELSE IF (mode == 1) THEN
          vv = vv + beta * r**7 * (r - r0)**2 * cos(time)**4 * (12.d0 + 7 * pi**2 * r**2 + &
               (12.d0 - 7 * pi**2 * r**2) * cos(4*pi*z)) * sin(2*pi*z)**2 / &
               lambda**2
       ELSE IF (mode == 2) THEN
          vv = vv + beta * r**7 * (r - r0)**2 * cos(time)**4 * (5.d0 + 4 * pi**2 * r**2 + &
               (5.d0 - 4 * pi**2 * r**2) * cos(4*pi*z)) * sin(2*pi*z)**2 / &
               lambda**2
       ELSE IF (mode == 3) THEN
          vv = vv + beta * 2 * pi**2 * r**9 * (r - r0)**2 * cos(time)**4 * sin(2*pi*z)**4 / &
               lambda**2
       ELSE IF (mode == 4) THEN
          vv = vv - beta * r**7 * (r - r0)**2 * cos(time)**4 * sin(4*pi*z)**2 / &
               (4 * lambda**2)
    ...
    ELSE IF (TYPE == 4) THEN
       IF (mode == 1) THEN
          vv = vv + beta * r**7 * (r - r0)**2 * cos(time)**4 * (25.d0 + 8 * pi**2 * r**2 + &
               (25.d0 - 8 * pi**2 * r**2) * cos(4*pi*z)) * sin(2*pi*z)**2 / &
               (4 * lambda**2)
       ELSE IF (mode == 2) THEN
          vv = vv + beta * r**7 * (r - r0)**2 * cos(time)**4 * (61.d0 + 24 * pi**2 * r**2 + &
               (61.d0 - 24 * pi**2 * r**2) * cos(4*pi*z)) * sin(2*pi*z)**2 / &
               (8 * lambda**2)
       ELSE IF (mode == 3) THEN
          vv = vv + beta * r**7 * (r - r0)**2 * cos(time)**4 * (17.d0 + 8 * pi**2 * r**2 + &
    ...
    ELSE IF (TYPE == 5) THEN
       IF (mode == 0) THEN
          vv = vv + beta * pi * r**8 * (-215.d0 + 136 * pi**2 * r**2) * (r - r0)**2 * cos(time)**4 * &
               cos(2*pi*z) * sin(2*pi*z)**3 / (4 * lambda**2)
       ELSE IF (mode == 1) THEN
          vv = vv + beta * 5 * pi * r**8 * (-11.d0 + 8 * pi**2 * r**2) * (r - r0)**2 * cos(time)**4 * &
               cos(2*pi*z) * sin(2*pi*z)**3 / lambda**2
       ELSE IF (mode == 2) THEN
          vv = vv + beta * 14 * pi * r**8 * (r - r0)**2 * cos(time)**4 * &
               cos(2*pi*z) * sin(2*pi*z)**3 / lambda**2
       ELSE IF (mode == 3) THEN
          vv = vv - beta * pi * r**8 * (-19.d0 + 8 * pi**2 * r**2) * (r - r0)**2 * cos(time)**4 * &
               cos(2*pi*z) * sin(2*pi*z)**3 / lambda**2
       ELSE IF (mode == 4) THEN
          vv = vv - beta * pi * r**8 * (-15.d0 + 8 * pi**2 * r**2) * (r - r0)**2 * cos(time)**4 * &
               cos(2*pi*z) * sin(2*pi*z)**3 / (4 * lambda**2)
       END IF
    ELSE IF (TYPE == 6) THEN
       IF (mode == 1) THEN
          vv = vv + beta * 8 * pi * r**8 * (-9.d0 + 5 * pi**2 * r**2) * (r - r0)**2 * cos(time)**4 * &
               cos(2*pi*z) * sin(2*pi*z)**3 / lambda**2
       ELSE IF (mode == 2) THEN
          vv = vv + beta * 4 * pi * r**8 * (-13.d0 + 8 * pi**2 * r**2) * (r - r0)**2 * cos(time)**4 * &
               cos(2*pi*z) * sin(2*pi*z)**3 / lambda**2
       ELSE IF (mode == 3) THEN
          vv = vv + beta * 8 * pi * r**8 * (-1.d0 + pi**2 * r**2) * (r - r0)**2 * cos(time)**4 * &
               cos(2*pi*z) * sin(2*pi*z)**3 / lambda**2
       ELSE IF (mode == 4) THEN
...
\endcode
<li>The function <tt>T_dchi_dT_coeff_law</tt> computes the coefficient \f$ T \frac{\partial \chi}{\partial T}(T) \f$ of the magnetic term in the temperature equaton. It takes a real \f$T\f$ and gives the real \f$ T \frac{\partial \chi}{\partial T}(T) \f$. We implement:
\code
vv = - 2 * inputs%mag_force_coefficient*temp**2
\endcode
</ol>
All the other subroutines present in the file <tt>condlim_test_38.f90</tt> are not used in this test.
...
We describe the data file of this test.
 It is called <tt>debug_data_test_38</tt> and can be found in the directory ($SFEMaNS_DIR)/MHD_DATA_TEST_CONV_PETSC.
<ol>
...
<li>We use two processors in the meridian section.
\code
...
6
\endcode
The Fourier modes are not detailed so the first six modes \f$0,1,...,5\f$ are solved.
<li>We use \f$3\f$ processors in Fourier space.
\code
===Number of processors in Fourier space
3
\endcode
It means that each processors is solving the problem for \f$6/3=2\f$ Fourier modes.
...
These quantities are computed at the final time \f$t=1\f$.
 ...
 These values of reference are in the last lines of the file <tt>debug_data_test_38</tt> in ($SFEMaNS_DIR)/MHD_DATA_TEST_CONV_PETSC. They are equal to:
\code
============================================
(SOLID_FLUID_10.FEM)
===Reference results
4.2196153524889061E-004  L2-norm of error on u / L2-norm of u exact
7.5583862715430730E-002  L2-norm of error on p / L2-norm of p exact
3.2149076277439141E-003  L2-norm of error on T / L2-norm of T exact
6.8368602434746998E-003  L2-norm of error on H / L2-norm of H exact
\endcode
...
<tr>
    <td align="center">
     @image html  fig_test_38_pre_tfin.png "Pressure in the plane y=0."
    </td>
</tr>
</table>
<table width="100%" align="center">
<tr>
    <td align="center">
    @image html  fig_test_38_vel_tfin.png  "Velocity magnitude in the plane y=0."
    </td>
</tr>
</table>
<table width="100%" align="center">
<tr>
    <td align="center">
     @image html  fig_test_38_temp_tfin.png "Temperature in the plane y=0."
    </td>
</tr>
</table>
<table width="100%" align="center">
<tr>
    <td align="center">
     @image html  fig_test_38_mag_tfin.png "Magnetic field magnitude in the plane y=0."
    </td>
</tr>
</table>
 */
