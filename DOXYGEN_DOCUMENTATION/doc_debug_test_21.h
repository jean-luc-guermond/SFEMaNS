...
/**
 * @page doc_debug_test_21 Test 21: MHD with variable density and electrical conductivity

<h3>Introduction</h3>
In this example, we check the correctness of SFEMaNS for a magnetohydrodynamic problem with variable electrical conductivity involving Dirichlet boundary conditions. The mass equation is not approximated. We consider a level set, solution of the same advection equation, that is used to reconstruct the density and the other fluid's properties.
...
We recontruct the density, dynamical viscosity and electrical conductivity as follows:
@f{align*}
...
@f{align*}
\partial_t\bm + \DIV(\bm\otimes\bu) - \frac{2}{\Re} \DIV(\eta \epsilon(\bu)) +\GRAD p    
 &=\bef + (\ROT \textbf{H}) \times (\mu^c \textbf{H}),
\\ \DIV \bu &= 0, \\
...
+ \nabla \times \left(\frac{1}{\Rm \sigma}\mathbf{j} \right), \\
...
The data are the source terms \f$f_\varphi\f$, \f$\bef\f$ and \f$\textbf{j}\f$, the boundary data \f$\bu_{\text{bdy}}\f$,
the initial datas \f$\bu_0\f$ and \f$p_0\f$. The parameter \f$\Re\f$ is the kinetic Reynolds number$, the magnetic Reynolds number \f$\Rm\f$, the magnetic permeability \f$\mu^c\f$, the densities \f$\rho_i\f$, the dynamical viscosities \f$\eta_i\f$ and the electrical conductivities  \f$\sigma_i\f$.
...
\\H_z(r,\theta,z,t) &=  - 4 r^2 z ,
\\ \varphi(r,\theta,z,t) &=1 + z + r \cos(\theta),
\\ \rho(r,\theta,z,t) &=2 + z + r \cos(\theta),
\\ \eta(r,\theta,z,t) &=1,
\\ \sigma(r,\theta,z,t) &=2 + z + r \cos(\theta).
@f}
where the source terms \f$f_\varphi\f$, \f$\bef\f$, \f$\textbf{j}\f$ and the boundary data \f$ \bu_{\text{bdy}}\f$ are computed accordingly.
...
The initial conditions, boundary conditions and the forcing terms are set in the file <tt>condlim_test_21.f90</tt>.
...
<li>The subroutine <tt>init_maxwell</tt> initializes the magnetic field at the time\f$-dt\f$ and \f$0\f$ with \f$dt\f$ being the time step. It is done by using the function Hexact.
\code
    time = -dt
    DO k=1,6
       DO i=1, SIZE(list_mode)
    ...
    ENDDO
\endcode
We note the scalar potential is also initialized with the function Phiexact in the case that nb_dom_phi>0. As there is no scalar potential in this test, we have nb_dom_phi=0.
...
It is not used in this test so we stop the code if this subroutine is called as follows:
\code
   vv=0.d0
...
Since \f$\partial_t (\mu^c \textbf{H})=0 \f$ and \f$ \bu=0 \f$, this source term has to satisfy
 the relation \f$\textbf{j}= \ROT(\textbf{H}) \f$.
\code
...
All the other subroutines present in the file <tt>condlim_test_21.f90</tt> are not used in this test.
...
We describe the data file of this test.
 It is called <tt>debug_data_test_21</tt> and can be found in the following directory: ($SFEMaNS_DIR)/MHD_DATA_TEST_CONV_PETSC.
<ol>
...
0.d0
\endcode
It means the entropy viscosity is zero.
...
<li>We define the parameters \f$(\sigma_1,\sigma_2)\f$ used to reconstruct the electrical conductivity.
\code
...
<li>We use the magnetic field B as dependent variable for the Maxwell equations.
\code
===Solve Maxwell with H (true) or B (false)?
.f.
\endcode
<li>We set the number of domains and their label, see the files associated to the generation of the mesh, where the code approximates the magnetic field.
\code
...
1.d0
\endcode
This value is not used by the code since the conductivity is reconstructed with the level set. These lines are required. They allow to approximate the Maxwell equations is another domain than the fluid one  with a conductivity sets in the data.
...
<li>The L2 norm of the error on the the magnetic field.
</ol>
These quantities are computed at the final time \f$t=1\f$.
 ...
 These values of reference are in the last lines of the file <tt>debug_data_test_21</tt> in the directory ($SFEMaNS_DIR)/MHD_DATA_TEST_CONV_PETSC. They are equal to:
\code
============================================
(Mesh_10_form.FEM)
===Reference results
1.3906183032940818E-005  L2 error on velocity
3.0086983840053983E-006  L2 error on pressure
6.7632134413749881E-006  L2 error on level set
1.8335168044284966E-005  L2 error on Magnetic field
\endcode
...
<tr>
    <td align="center">
     @image html  fig_test_21_level_set_tfin.png "Level set in the plane plane y=0."
    </td>
    <td align="center">
    @image html  fig_test_21_pre_tfin.png  "Presure in the plane plane y=0."
    </td>
</tr>
<tr>
    <td align="center">
     @image html  fig_test_21_vel_tfin.png "Velocity magnitude in the plane plane y=0."
    </td>
    <td align="center">
    @image html  fig_test_21_mag_tfin.png  "Magnetic field magnitude in the plane plane y=0."
    </td>
</tr>
</table>
 */
