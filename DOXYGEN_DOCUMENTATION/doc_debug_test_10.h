...
/**
 * @page doc_debug_test_10 Test 10: Maxwell P2
...
\end{cases}
\f}
in the conducting domain \f$\Omega_1= \{ (r,\theta,z) \in {R}^3 : (r,\theta,z) \in [0,0.5] \times [0,2\pi) \times [0,1]\} \f$ with \f$\Gamma_1= \partial \Omega_1\f$.
...
The initial conditions, boundary conditions and the forcing term \f$\textbf{j}\f$ in the Maxwell
 equations are set in the file <tt>condlim_test_10.f90</tt>.
...
<li>The subroutine <tt>init_maxwell</tt> initializes the magnetic field and the scalar potential 
 ...
 It is done by using the functions Hexact:
\code
    time = -dt
    DO k=1,6
    ...
    ENDDO
\endcode
The scalar potential \f$\phi\f$ is initialized but we note it will not be used for this example.
...
All the other subroutines present in the file <tt>condlim_test_10.f90</tt> are not used in this test.
...
We describe the data file of this test.
 It is called <tt>debug_data_test_10</tt> and can be found in the following directory: ($SFEMaNS_DIR)/MHD_DATA_TEST_CONV_PETSC.
<ol>
...
<li>The L2 norm of the divergence of the magnetic field \f$\textbf{B}=\mu\bH\f$.
 ...
 These values of reference are in the last lines of the file <tt>debug_data_test_10</tt> in the directory ($SFEMaNS_DIR)/MHD_DATA_TEST_CONV_PETSC. They are equal to:
\code
============================================
Mesh_10_form.FEM, P2
===Reference results
...
<tr>
    <td align="center">
     @image html  fig_test_10_mag_tfin.png "Magnetic field magnitude in the plane plane y=0."
    </td>
</tr>
</table>
 */
